...
Date: 2026-04-12 (multiple same-day revisions); contribution-ordering revision 2026-04-29
Paper state: draft-complete (18pp after rev2 polish + headline reorder)

## 2026-04-29 revision (later): two-scale framing in §1

After the contribution-ordering reorder above, the thesis was further
sharpened from "necessary but not sufficient" to a positive structural
claim: cipher map confidentiality decomposes into two scales that do
not reduce to each other.

- **Marginal scale** (single cipher map): representation-uniformity
  parameter delta controls per-cipher-value entropy ratio via Fannes-
  Audenaert; two constructions reduce delta with explicit costs.
- **Compositional scale** (chains): observing multiple evaluations on
  a shared cipher value lets the untrusted machine recover the latent
  joint at parametric rate O(|Y_1||Y_2|/xi^2), with mutual information
  preserved exactly. Independent of delta.

The framing reads the compositional channel as **intrinsic to the
framework's composability, not a bug**: the operational feature that
lets the untrusted machine chain evaluations blindly is the same
feature that creates the compositional channel. So both scales must be
engineered independently; engineering at one does not engineer at the
other.

Edits landed (per AskUserQuestion answers):
- Abstract: kept current (two-scale frame implicit in narrative arc).
- Section 1: inserted a 'Our central observation' paragraph after the
  cipher-maps setup. The existing Fannes-bridge paragraph reframed as
  'Marginal scale.' (the scale with delta -> e). The existing
  compositional leakage paragraph reframed as 'Compositional scale.'
  with the 'compositional channel is intrinsic to composability'
  framing.
- Section 5 renamed from 'Compositional Confidentiality' to 'The
  Compositional Scale'.
- Section 5 opener rewritten to pivot off scales rather than 'two
  effects.'

Sections 4 ('Constructions for Reducing delta') and 3 ('The
Confidentiality Measure') are not renamed; their function in the new
frame is implicitly the marginal-scale machinery.

No mathematical content changed in this revision. Build remains 18
pages, zero warnings.

## 2026-04-29 revision: contribution ordering

Per rev2 review (`.papermill/reviews/2026-04-12-rev2/review.md`,
Suggestion 2), promoted Theorem 5.1 (compositional leakage) above the
Fannes bridge in the contribution ordering. Rationale:

- Thm 5.1 parts (2)-(4) now carry quantitative content (MI preservation,
  parametric rate, joint TV bound on optimal delta_3) after rev2 polish.
- The Fannes bridge is a textbook application of Fannes-Audenaert
  continuity; framing novelty without mathematical novelty.
- Thm 5.1 is the paper's distinctive contribution: it is the reason
  marginal-uniformity engineering is not the whole story, and the
  proof is paper-specific.

Edits landed:
- Abstract: ordering is now (entropy ratio + Fannes bridge as setup) ->
  (constructions) -> (compositional leakage as headline punchline).
  Compositional leakage moved from middle of abstract to its end for
  emphasis.
- Section 1 framing: compositional leakage paragraph rewritten to
  declare itself "the paper's main result" and "headline contribution"
  rather than "a distinct compositional contribution."
- Contributions list: reordered to lead with compositional leakage
  (now numbered 1, was 3); Fannes bridge is now 2 (was 1); two
  constructions is 3 (was 2); practical measurement is 4 (unchanged).
- Section 2 Related Work QIF paragraph: swapped (i) and (ii); now
  leads with compositional leakage as the new contribution.
- Section 8 Conclusion: rewritten to lead with compositional leakage
  as headline; Fannes bridge demoted to "to make the marginal target
  itself operational, ..."

No mathematical content changed. Build remains 18 pages, zero warnings.
