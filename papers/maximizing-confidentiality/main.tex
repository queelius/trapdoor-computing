...
\cite[Sec.~8]{towell2026cipher}.  We use it but do not re-prove it
here.
...
\item \textbf{Compositional leakage at the optimal rate (main result).}
  ...
  matching minimax lower bound is via Assouad's lemma.  No
...
$(\eta, \varepsilon, \mu, \delta)$ and a composition discipline under
...
\begin{remark}[Two normalizers for $H^*$]
\label{rem:hstar}
The theorem uses two values of $H^*$ for two different questions.  The
\emph{ambient} entropy ratio normalizes by $H^* = n$, the entropy of
the uniform distribution on the full cipher space $\B^n$; this is the
quantity the Fannes bound (part 3) controls and the canonical marginal
measure (it is what ``the entropy ratio'' means without qualification).
The \emph{effective-alphabet} ratio in part (2) normalizes by
$H^* = \log_2|X|$, the entropy of the uniform distribution over the
$|X|$ in-domain values, and answers a narrower question: how much of
the \emph{achievable} entropy a simple substitution captures.  When
$|X| \ll 2^n$ the two differ; the worked entropy ratios reported in
the examples and case study of \S\ref{sec:levers} and
\S\ref{sec:experiments} use the effective-alphabet normalizer
$\log_2|X|$, since the cipher space is sized to the vocabulary.
\end{remark}
...
chains.  This is inherited from~\cite[Sec.~7.4]{towell2026cipher} and
...
This is the standard Assouad minimax bound for distribution estimation
in TV, applied to the joint support $Y_1 \times Y_2$.\footnote{The
...
$\{(v_i^+, v_i^-)\}_{i=1}^{m/2}$.  For $s \in \{\pm 1\}^{m/2}$ and
$\varepsilon$ a small constant times the target $\xi$, define
$P^{(s)}$ by
\[
...
$\TV(P^{(s)}, P^{(s')}) = (2\varepsilon/m)\,d_H(s, s')$ where $d_H$ is
Hamming distance (each differing coordinate moves mass $\varepsilon/m$
at $v_i^+$ and $\varepsilon/m$ at $v_i^-$, contributing $2\varepsilon/m$
to TV).  Assouad's lemma applied to this $2^{m/2}$-packing, with KL
divergence between adjacent hypotheses bounded by
