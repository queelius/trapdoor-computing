...
three streams whose entropy rates are known analytically.  Each
stream uses 16-bit cipher values ($n = 16$, byte-aligned for gzip)
and is averaged over $5$ random seeds.

\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{compression_validation}
\caption{Compression-based entropy estimation against analytically
known entropy rates ($n = 16$, $N = 10^5$, $5$ seeds, error bars
$\pm 1\sigma$).
\textbf{Left}: first-order Markov stream with auto-correlation
parameter $p$, entropy rate $H_{\mathrm{rate}} = (1-p)n$.
\textbf{Right}: real Zipf queries ($s=1$, $m = 10\,000$) mixed
...
The estimator tracks $H_{\mathrm{mix}}$ from $\rho = 0.1$
($\hat H = 15.90$, predicted $15.82$) to $\rho = 1.0$ pure Zipf
($\hat H = 11.72$, predicted $9.53$).  Bias grows monotonically
with $\rho$ because Zipf queries lack the temporal structure that
LZ77 can exploit; in the pure-Zipf regime gzip degenerates into a
byte-level Huffman coder, and the byte-pair entropy of 16-bit Zipf
samples is higher than the 14-bit symbol entropy of the underlying
distribution.  Adding filler dilutes this confound.

\paragraph{Operational reading.}
Compression estimators give a useful relative measurement of
confidentiality changes (noise-injection effects, before-vs-after
comparisons), and the bias direction matches Proposition~\ref{prop:compression}.
For absolute Shannon-entropy measurements on cipher-value streams
that lack temporal structure, a model-based estimator (e.g.,
context-tree weighting~\cite{cover2006elements}) is preferable.
Operators tracking confidentiality over time should fix the
