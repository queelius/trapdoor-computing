...
Each Boolean operation is a cipher map over the product of its input
cipher spaces.
For cipher Boolean type with $|T \cup F| = m$ values,
AND requires $m^2$ entries.
More generally, a cipher map $\fhat : \cipher{X_1} \times \cdots
\times \cipher{X_k} \to \cipher{Y}$ requires
$\prod_i |\cipher{X_i}|$ entries.
Deeper composition chains multiply the number of cipher maps (one per
level), each with its own $m^2$ (or $\prod |\cipher{X_i}|$) domain.
