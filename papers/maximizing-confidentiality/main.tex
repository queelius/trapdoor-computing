...
and that no per-cipher-map parameter---$\delta$ included---moves it.
...
access distribution), whereas we prove that the same $\delta$ which bounds
the marginal provably does not bound joint leakage under composition---a
quantitative separation, with a matching minimax rate, that the
