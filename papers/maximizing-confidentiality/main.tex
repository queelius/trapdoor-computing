...
\emph{information-theoretically optimal} rate
$\Theta(|Y_1|\cdot|Y_2|/\xi^2)$ (matching upper bound from plug-in
estimation, matching lower bound via Le Cam's two-point method).  The
...
injection) reduce the adversary's effective advantage.  Experimental validation on
...
Theorem~\ref{thm:comp-leakage}; matching lower bound via Le Cam's
two-point method in Theorem~\ref{thm:comp-leakage-lb}); mutual
information is preserved exactly,
$I(\fhat_1(C); \fhat_2(C)) = I(f_1(X); f_2(X))$.  No per-cipher-map
parameter (including arbitrarily small $\delta$) changes the rate.
...
\begin{enumerate}
\item \textbf{Compositional leakage at the optimal rate (main result).}
  Theorems~\ref{thm:comp-leakage} and~\ref{thm:comp-leakage-lb}
  together establish that when the untrusted machine observes
  ...
  matching minimax lower bound is via Le Cam's two-point method.  No
  per-cipher-map parameter (including arbitrarily small $\delta$)
  changes the rate, so reducing $\delta$ is necessary but not
  ...
  inherited from~\cite{towell2026cipher} (\S\ref{sec:composition}).
...
result showing that $\delta$-uniform marginals are insufficient when
shared cipher values recur across evaluations
(Theorems~\ref{thm:comp-leakage} and~\ref{thm:comp-leakage-lb})
establishing the information-theoretically optimal rate
$\Theta(|Y_1|\cdot|Y_2|/\xi^2)$ via matching upper and lower bounds,
the paper's main result, and (ii) the \emph{Fannes bridge} connecting
the representation-uniformity parameter $\delta$ of cipher maps to
the entropy ratio
...
plug-in estimation and matching lower bound via Le Cam's two-point
method.  No per-cipher-map parameter changes the rate, so marginal
