...
\graphicspath{{img/}}
\usepackage{hyperref}
\usepackage{xurl}  % allow long URLs to break, avoiding margin overflow
\hypersetup{
  hypertexnames=false,
...
\item \textbf{Noise injection.}  The trusted machine mixes $R$ filler
  ...
  Effect: Fisher information for any correlation estimator is reduced
  by $\rho^2 = (N/(N+R))^2$ (Theorem~\ref{thm:noise-dilution}, new
  here).
...
\texttt{experiments/compositional\_rate.py}.  All scripts read no
...
section.  We validate both by Monte Carlo
...
part~(3): an adversary estimating any parameter of $D$ from the mixed
stream pays a factor $\rho^{-2}$ in samples.

\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{compositional_rate}
\caption{Monte Carlo validation ($40$ seeds/cell).
\textbf{Left}: joint-recovery error $\TV(\hat P, P)$ versus $N$,
log-log, one line per support size $m = |Y_1||Y_2|$; fitted slopes
near $-1/2$.
\textbf{Centre}: the same data collapses onto
...
\textbf{Right}: noise-injection Fisher reduction $I_M/I_D \propto
