...
\paragraph{Space cost of joint encoding.}
Merging cipher nodes to prevent correlation leakage increases the domain
of the resulting cipher map.
Consider $h(f(x), g(x))$ where $f$ and $g$ share input $x$.
With separate cipher maps $\fhat$, $\hat{g}$, and $\hat{h}$, the
untrusted machine sees both $\fhat(c)$ and $\hat{g}(c)$, leaking the
correlation between $f$ and $g$.
The alternative is a single cipher map for the composition:
$\widehat{h \circ (f, g)}(c)$ maps $c$ directly to
$h(f(x), g(x))$ with no intermediate values exposed.
The domain is $|X|$ (same as the components), but the construction must
evaluate $f$, $g$, and $h$ for every $x \in X$ at build time.
If instead we partially merge $f$ and $g$ into a tuple cipher map
$\widehat{(f,g)}(c) \to \cipher{Y_f \times Y_g}$, the codomain is the
product space $|Y_f| \times |Y_g|$, which grows exponentially in the
number of merged components.
This is the fundamental space cost of the encoding granularity principle:
