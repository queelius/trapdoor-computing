...
% Active algorithm keywords (dead ones removed in notation pass)
\SetKwFunction{BL}{BL}
...
\newcommand{\catop}{{+\!\!+}}
% Core notation (dead commands removed in notation pass)
...
We now formalize each component. We write $\cisb_n = \{0,1\}^n$ for bit strings of length $n$, $\cisb = \{0,1\}^*$ for bit strings of any length, and $\BL(x)$ for the bit length of $x$. A \emph{confidential object} (e.g., a document) is one that only authorized search agents should comprehend.

The system satisfies \emph{information needs} (queries) of authorized search agents by retrieving relevant documents.
...
\begin{assumption}
\label{asm:trapdoor}
The one-way function $\hash \colon \cisb \mapsto \cisb_m$ approximates a random oracle (i.e., $\hash$ is a cryptographic hash function).
\end{assumption}
...
The \emph{adversary} is an untrusted agent that tries to extract confidential information about encrypted search activities. Following Kerckhoffs's principle, the system is secure even if everything except the \emph{secret} is known to the adversary.
\begin{assumption}
...
\begin{assumption}
The secret consists of a single \emph{secret key} used to generate trapdoors.
\end{assumption}
...
\end{assumption}
By Assumption~\ref{asm:obfuscator_secure}, plaintext queries reach the obfuscator without compromise.
...
We illustrate with two search agents.
\begin{example}
\label{ex:es_act}
...
To transmit hidden queries and result sets across the untrusted channel, we use uniquely decodable encoders $\Encode \colon [\mathbb{Y}] \mapsto \cisb$ (for trapdoor sets) and $\Encode \colon [\mathbb{D}] \mapsto \cisb$ (for document identifier sets).
...
We now analyze the information disclosure of these streams and explore strategies to increase their entropy.
\section{Probabilistic model}
\label{sec:probabilistic}
We model encrypted search activities as random processes. We use standard notation: $\pmf_{\rv{X}}(x)$ is the probability mass function of discrete random variable $\rv{X}$, and $\pmf_{\rv{X_1}, \ldots, \rv{X_n}}(x_1, \ldots, x_n)$ is the joint PMF.
...
The distribution of inter-arrival times can be estimated by time series methods such as exponential smoothing.

Suppose there are $k$ search agents generating queries, with varying frequencies.
\begin{definition}
The search agent responsible for the \jth query is a discrete random variable $\rvsa[j]$ with support
\begin{equation}
...
\begin{definition}
The \jth random bag-of-words in the plaintext query time series is $\rvqb[j]$ with support
\begin{equation}
    \left\{
...
\begin{definition}
The \emph{random tuple} of the \jth plaintext query is
\begin{equation}
...
\begin{definition}
The inter-arrival time between the $(j-1)$-th and \jth hidden query is a discrete random variable $\rvhts[j]$ with support
\begin{equation}
...
The obfuscator may inject artificial queries to perturb the stream, so the agent responsible for each query is uncertain.
\begin{definition}
The search agent responsible for the \jth hidden query is a discrete random variable $\rvhsa[j]$ with support
\begin{equation}
...
There are $N$ \emph{unique} confidential objects (and $N' - N$ obfuscated or artificial objects) for a total of $N'$ objects. The random query $\rvhqb[j]$ induces a distribution over result sets.
\begin{definition}
The \kth random result set $\rvhrs[k]$ has support given by the power set of
\begin{equation}
...
The \emph{random tuple} of the \jth hidden query and result set is
\begin{equation}
...
The greater the entropy of the hidden query stream, the fewer patterns an adversary can exploit. We use standard Shannon entropy $\entropy(\rv{X}) = -\sum_x \pmf_{\rv{X}}(x) \log_2 \pmf_{\rv{X}}(x)$, conditional entropy $\entropy(\rv{X} \given \rv{Y})$, and joint entropy decomposed via the chain rule:
\begin{equation}
...
The joint entropy is at most the sum of marginal entropies:
\begin{equation}
...
\begin{definition}
The \emph{confidentiality efficiency} of an encrypted search system with entropy $\entropy_n$ is
\begin{equation}
...
Since the document collection is immutable, a given hidden query always maps to the same result set (the result set is degenerate given the query). Consequently,
\begin{equation}
...
A full set-theoretic query model\cite{manning2008introduction} reveals more about information needs, so set-theoretic operations beyond intersection should be applied on the trusted machine rather than the untrusted provider.
...
The maximum entropy of $(\rv{L}, \rv{Y})$ is:
\begin{theorem}
\begin{equation}
...
%\input{perturbed_fake_query.tex}
%\end{figure}
...
The memoryless property of the exponential distribution reflects its maximum entropy. If the support is constrained to $[0, 2/\lambda]$, the uniform distribution also achieves entropy $\ln \lambda$ with $\expectation[\rvhts] = 1/\lambda$.
...
By the chain rule, $\entropy(\rv{X}_1, \ldots, \rv{X}_n) = \sum_{i=1}^{n} \entropy(\rv{X}_i \mid \rv{X}_{<i})$, which reduces to $\sum_i \entropy(\rv{X}_i)$ under independence and to $n \entropy(\rv{X})$ under i.i.d.
...
\midrule
$\rv{X}$, $x$ & Random variable, its realization\\
$\pmf_{\rv{X}}(x)$, $\pdf_{\rv{X}}(x)$ & Probability mass/density function\\
$\entropy(\rv{X})$, $\entropy^*(\rv{X})$ & Shannon entropy, maximum entropy\\
$\info(\rv{X}; \rv{Y})$ & Mutual information\\
$\expectation[\rv{X}]$ & Expected value\\
$\BL(x)$ & Bit length of $x$\\
$e = \entropy/\entropy^*$ & Confidentiality efficiency\\
\midrule
$\qb$, $\hqb$ & Plaintext query, hidden query\\
$\vec{d}$, $\hrs$ & Document, result set\\
$\lambda$ & Query arrival rate\\
$\mu$ & Mean trapdoors per query\\
$k$, $m$, $N$ & Search agents, vocabulary size, documents\\
\bottomrule
\end{tabular}
\end{table}
