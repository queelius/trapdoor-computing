...
This paper develops the quantitative theory.  The central idea, first
explored in~\cite{towell2017maxconf} before the cipher map formalism
existed, is the \emph{entropy ratio}: the ratio of observed entropy in
the cipher map output stream to the maximum entropy achievable under
system constraints.  We ground this idea in the cipher map framework,
...
\item Theorems connecting each of the three levers (noise, multiplicity,
  granularity) to the entropy ratio, with explicit cost/benefit trade-offs
  (\S\ref{sec:levers}).

\item Analysis of confidentiality under cipher map composition, including
  FPR compounding through Boolean chains and correlation leakage from
  shared variables (\S\ref{sec:composition}).
...
indistinguishability.  We do not write $\mathsf{Adv}$, $\mathsf{Sim}$,
or $\mathsf{PPT}$.  The framework is information-theoretic throughout.
...
\item[Representation uniformity ($\delta$-bounded).]  The marginal
  distribution of cipher values is $\delta$-close (in total variation
  distance) to uniform over $\B^n$.

\item[Correctness ($\eta$-bounded).]  For random in-domain $x$ and
  random representation index $k$,
  $\Pr[\dec(\fhat(\enc(x,k))) \neq f(x)] \leq \eta$.
...
\paragraph{Notation.}
Throughout, $H(\cdot)$ denotes Shannon entropy in bits, $H(X \mid Y)$
is conditional entropy, and $I(X; Y)$ is mutual information.  We use
$D$ for the distribution of queries over the domain $X$, and $Q$ for
the induced distribution over cipher values.  All logarithms are base 2.
...
For part (3): by Pinsker's inequality,
...
overhead is approximately 518 additional trapdoors per
document~\cite{towell2017maxconf}.
\end{example}
...
and a starting cipher value $c$, the orbit
$\orbitF_F(c)$ is the set of all cipher values reachable by composing
operations in $F$.  The confidentiality is bounded
by~\cite[Thm.~5.3]{towell2026algebraic}:
\begin{equation}
\mathrm{conf}_F(c) \geq 1 - \frac{|\orbitF_F(c)|}{2^n}.
\end{equation}
Fewer exposed operations means smaller orbit means higher
...
For Boolean-valued cipher maps (set membership predicates), the
...
For $k$ independent cipher set membership tests composed via Boolean
operations:
\begin{align}
...
same recall (true positives pass all tests).  From a confidentiality
perspective, AND chains reduce the effective noise level: with
$p_T = 0.05$ and $k = 3$, the FPR drops to $0.05^3 \approx 0.000125$,
meaning almost all decoded ``true'' outputs correspond to genuine
members.  This makes the system more predictable to the adversary (lower
$e$) in exchange for higher precision.

OR chains increase the noise level but reduce recall.  This makes the
system less predictable (higher $e$) but also less useful (more false
...
\begin{proof}
An optimal compressor $\mathrm{Compress}^*$ produces output whose
expected bit length equals $H(C_1, \ldots, C_N)$ by Shannon's source
coding theorem.  Any sub-optimal compressor produces output at least as
long.  The particular encoding $\mathrm{Encode}$ is irrelevant: the
compressor compensates for the encoding overhead (as long as the encoding
