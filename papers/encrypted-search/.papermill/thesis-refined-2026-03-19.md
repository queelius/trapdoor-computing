...
**Question**: Is this a standalone paper, or a section of cipher-maps?
...
The paper as it stands is a vocabulary dictionary: five definitions mapping encrypted search terms to cipher map terms. The cipher-maps paper already has a Section 7.2 ("Encrypted Search Systems") and Section 7.3 ("Privacy-Preserving Search") that cover the same ground in bullet-point form. The encrypted-search paper adds more careful definitions, but the contribution is still definitional -- it establishes a correspondence without proving anything new follows from it.
...
## 2. Absorption into cipher-maps

The cipher-maps paper already has:
...
**Replace Sections 7.2 and 7.3 of cipher-maps with a single expanded subsection** incorporating:
...
This would strengthen cipher-maps's applications section and avoid publishing a paper whose sole contribution is a vocabulary mapping.

**Cost of absorption**: The encrypted-search paper's framing that encrypted search is "one application among many" gets lost slightly, because cipher-maps already makes this point. But the precise vocabulary mapping adds concrete value to the applications section.
...
**Short-term**: Absorb the vocabulary mapping into cipher-maps (replacing the thin Sections 7.2-7.3). This is a net improvement to the unified paper at no cost.
...
**Bottom line**: The paper as written is not paper-worthy. The vocabulary mapping is correct but trivial. Either add a theorem (plausible deniability is the best candidate) or absorb into cipher-maps.
