...
We write $\{0,1\}^n$ for bit strings of length $n$, $\{0,1\}^*$ for the set of all finite bit strings, and $\epsilon$ for the empty string. We write $x \cat y$ for concatenation, $\BL(x)$ for the bit length of $x$, and $\mathrm{trunc}(x,k)$ for the first $k$ bits of $x$.

We identify bit strings with natural numbers via the standard bijection
\begin{equation}
\label{def:mapping}
b_1 \ldots b_m \longleftrightarrow 2^m + \sum_{j=1}^{m} 2^{m-j}\, b_j,
\end{equation}
writing $n'$ for the bit string of a natural number $n$ and $x'$ for the natural number of a bit string $x$. Note that $\BL(n') = \lfloor \log_2 n \rfloor$.

\subsection{Random oracle model}

\begin{assumption}
\label{asm:ro_approx}
The hash function $\mathrm{hash} : \{0,1\}^* \to \{0,1\}^\infty$ is a random oracle: for each input, the output is an independently and uniformly distributed infinite bit string.
\end{assumption}
