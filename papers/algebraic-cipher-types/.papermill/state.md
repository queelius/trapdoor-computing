...
thesis:
  ...
  refined: 2026-06-04
  previous_claim: "Cipher types form an algebra: the standard type constructors (void, unit, product, sum, exponential) build complex cipher types from simpler ones, and each constructor induces a specific confidentiality cost absent in plaintext programming. Products leak correlations; sums force an impossibility (tag hiding and untrusted pattern matching cannot coexist); the cipher exponential is the cipher map abstraction itself. A uniform information-theoretic bound holds across the algebra (H(X | view) >= H(X) - log_2 |orbit|), and a typed-composition discipline turns this bound into a design-time budget realized concretely as Python expression-tree decompositions with @cipher_node cut points. (2026-05-04 version; the realization clause was removed 2026-06-03.)"
  refinement_notes: "Two refinements since the original. (2026-05-04) Restored the algebraic framing promised by the title: across review rounds 2026-04-08 to 2026-04-30 the sum-type impossibility had drifted to the headline ('culminating in...'), underplaying the §4 algebra; the 05-04 pass made the algebra primary, lifted product/exponential trade-offs to Propositions alongside sum, and added the cipher-type-algebra summary table. (2026-06-04, recording the 2026-06-03 scope change) The claim's realization clause ('realized concretely as expression-tree decompositions with @cipher_node cut points') was removed: whole-program realization is a distinct problem that moved to a companion paper (towell2026cipherprog), and single-cipher-map construction belongs to the foundation paper (cited, not re-derived). The claim now ends on instantiation + evaluation + partial filtering, matching the body. This is a SCOPE refinement (what the paper claims to do), not a reframing of the algebra thesis, which is unchanged. The earlier 2026-04-12 claim ('Every algebraic operation... enlarges their orbit closure...') and the 2026-03-28 original are preserved below and in git/review_history."
  earlier_claim_2026_04_12: "Every algebraic operation the untrusted machine can perform on cipher values enlarges their orbit closure and, by information-theoretic bound, reduces confidentiality. Each algebraic type constructor (product, sum, exponential) therefore induces a quantifiable confidentiality cost, culminating in a sum-type impossibility (tag hiding and untrusted pattern matching cannot coexist), and a typed cipher-program discipline, realized concretely via expression-tree cut points, turns those costs into a design-time budget."
...
- 2026-06-04 (thesis pass): Confirmed the claim and novelty still match the
  body after the 2026-06-03 scope restructure; no rewrite needed. Fixed
  lineage bookkeeping (refined 2026-05-04 -> 2026-06-04; rotated
  previous_claim to the 05-04 version; preserved the 04-12 claim under
  earlier_claim_2026_04_12). Crystallized one-sentence central claim for the
  abstract/outline: "Confidentiality in trapdoor computing is a property of
  the type structure: each algebraic type constructor imposes a specific,
  unavoidable information-theoretic leakage, and orbit closure makes the
  leakage of any composition computable and boundable at design time."
  DECISION (author, option a): partial filtering stays a DEMONSTRATION
  folded into contribution (4), NOT promoted to a fifth contribution. The
  paper is theory-forward and content-complete; promoting an analytical-only
  result would invite a "where is the evaluation?" ask and require a rerun
  in src/cipher-maps. Revisit only if partial filtering is later measured.
  Headline result remains the sum-type impossibility (Thm 4.2), positioned
  as one of three constructor costs inside the algebra.
