...
For every binary constructor, the choice between joint and
component-wise encoding is a continuous spectrum
(\Cref{rem:sum-vs-product}, the entanglement parameter $p$
of~\cite[Sec.~9]{towell2026cipher}), and the constructor determines
\emph{what} is on each side of the spectrum.
The product spectrum runs from full correlation hiding to
correlation-leaking projections; the sum spectrum runs from full
tag hiding to tag-leaking pattern matching; the exponential
spectrum runs from a single product cipher map to independent
component cipher maps.
