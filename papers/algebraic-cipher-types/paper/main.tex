...
For a finite type $X$, the \emph{cipher type} $\cipher{X} \subseteq \B^n$
is the set of valid cipher values encoding elements of $X$:
\[
\cipher{X} \;=\; \{\, \enc(x, k) \;:\; x \in X,\; 0 \leq k < K(x) \,\},
\]
where $K(x) \geq 1$ is the number of representations of $x$.
We use $\cipher{X}$ throughout to denote the type of cipher values for
$X$, distinct from the latent type $X$ itself.
\end{definition}
...
properties.
We use the cipher-type notation $\cipher{X}$ from
\Cref{def:cipher-type} throughout.
...
We write the TM transition function as $\tau_T$ rather than the
conventional $\delta$ to avoid collision with the total-variation
parameter $\delta$ of representation uniformity used throughout
\Cref{sec:prelim,sec:orbit}.
...
halting state $q_\mathrm{halt}$, and a cipher map $\hat{\tau}_T$ for
the transition function
$\tau_T : Q \times \Gamma \to Q \times \Gamma \times \{L, R, S\}$.
...
At each step the untrusted machine applies $\hat{\tau}_T$ to the
...
The untrusted machine writes $c'$ back to the current cell and then
shifts the head marker by applying two auxiliary unary cipher maps:
$\hat{\mathrm{clear}} : \cipher{Q \times \Gamma} \to \cipher{Q \times \Gamma}$
replaces the state component of the just-read cell with $\varnothing$,
and $\hat{\mathrm{set}}_q : \cipher{Q \times \Gamma} \to \cipher{Q \times \Gamma}$
(parameterized by the new state $q$ extracted from $c'$) places the
head into the neighbouring cell at offset $d$.
We treat the shift as a choice over a small finite family of unary
cipher maps, one per $(q, d)$ pair, rather than as a binary operation
on adjacent cells; this keeps every cipher map's domain at a single
cell.
...
A cipher TM stores three families of cipher maps over the cipher space
$\cipher{Q \times \Gamma}$, each with domain $|Q \times \Gamma|$ and
therefore space $O(|Q| \cdot |\Gamma|)$ entries
(per~\cite[Sec.~6]{towell2026cipher}, plus $-\log_2 \varepsilon$ bits
per entry for the noise budget and the appropriate $H(\cdot)$ bits per
entry for value encoding):
$\hat{\tau}_T$ for the transition,
$\hat{\mathrm{clear}}$ for marking the just-read cell as headless,
and $|Q| \cdot |\{L, R, S\}|$ instances of $\hat{\mathrm{set}}_q$ for
placing the head in a neighbouring cell.
Total space is $O(|Q|^2 \cdot |\Gamma| \cdot |\{L,R,S\}|) = O(|Q|^2 \cdot |\Gamma|)$,
still independent of the input domain size $|X|$.
When $|X| \gg |Q|^2 \cdot |\Gamma|$, this is a qualitative improvement
...
\begin{proof}
With $\eta = 0$, $\hat{\tau}_T$ reproduces the plaintext Turing
...
$\tau_T$, with a self-loop: the cut-point output $(c', d)$ feeds the
next step's tape cell, which feeds $\tau_T$ again.
...
\paragraph{What bounds the orbit in each case.}
The two realizations rely on different orbit-bound arguments.
The expression-tree realization satisfies the typed-chain hypothesis
of \Cref{def:typed-chain} (distinct cipher spaces between cut points)
and inherits the bound $\sum_i N_i$ of \Cref{prop:typed-orbit}; for an
expression tree of depth $k$ with binary combinators starting from $m$
distinct latent inputs, this gives $\sum_{i=0}^{k} m^{2^i}$.
The cipher TM does \emph{not} satisfy the typed-chain hypothesis: it
reuses a single cipher space $\cipher{Q \times \Gamma}$ at every step,
a self-loop rather than a chain.
Its orbit is bounded by a separate, simpler argument: a deterministic
TM produces a single trajectory of length $T$, and an $\eta = 0$ cipher
TM reproduces that trajectory exactly, so at most $T+1$ distinct cipher
values appear along the run (with $m$ distinct initial tape
configurations treated as a population, $m(T+1)$).
The two bounds coexist because the cut-point structure is a syntactic
abstraction over decompositions, not a single confidentiality theorem:
the syntactic structure says \emph{which subgraphs become cipher maps};
the orbit bound depends on whether composition is iterated through one
cipher space (TM, deterministic-trajectory bound) or distinct ones
(expression tree, typed-chain bound).

\begin{example}[Regex matching: same total cost, different leakage]
\label{ex:regex-cut-points}
Consider matching a fixed regular expression $R$ of size $r$ against a
string $x \in \Sigma^\ell$, with DFA state set $Q_R$ (in the worst case
$|Q_R| = 2^{O(r)}$ via subset construction; for many regexes much
smaller).
\emph{As a cipher TM,} $\Gamma = \Sigma \cup \{\sqcup\}$ and
$\hat{\tau}_T$ is a single cipher map of space
$O(|Q_R| \cdot |\Sigma|)$, evaluated $\ell$ times.
The orbit along a fixed input has size at most $\ell + 1$ by the
deterministic-trajectory bound.
Leakage: the head moves right $\ell$ times, revealing $\ell$.
\emph{As an expression tree under the natural typed-chain
decomposition,} we place $\ell$ cut points $\hat{\delta}_0, \ldots,
\hat{\delta}_{\ell-1}$ with $\hat{\delta}_i : \cipher{Q_R}_i \times
\cipher{\Sigma} \to \cipher{Q_R}_{i+1}$, each of space
$O(|Q_R| \cdot |\Sigma|)$, total $O(\ell \cdot |Q_R| \cdot |\Sigma|)$.
The orbit is bounded by $1 + \ell$ from \Cref{prop:typed-orbit} (single
state value at each level).
Leakage: the cipher state values $c_0, \ldots, c_\ell$ flow between cut
points, so an adversary observing the run sees an $(\ell+1)$-tuple of
intermediate cipher state values rather than just a head-movement count.
The two realizations have the same total construction cost (up to a
constant factor) and the same orbit bound; they differ in residual
\emph{leakage profile} (head-movement count for the TM, intermediate
state-value sequence for the expression tree) and in evaluation cost
(repeated lookups in one cipher map vs.\ a chain of $\ell$ lookups in
distinct cipher maps).
Cut-point placement therefore selects \emph{which structure leaks},
not \emph{whether the program fits in space}.
\end{example}
...
\end{tabular}
\end{center}
As expected from the PHF construction (perfect-hash backing gives
exact lookup on the construction domain), all three produce zero
errors on the full domain (150 inputs); we report this as a sanity
check, not a measured outcome.
