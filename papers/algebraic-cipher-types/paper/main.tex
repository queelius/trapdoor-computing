...
distribution over inputs that the trusted machine uses).
The untrusted machine's probing under $F$ yields an \emph{observation}
$\mathcal{V}_F(c)$: the cipher value its computation resolves to among
those reachable from $c$.
By construction $\mathcal{V}_F(c)$ takes values in $\orbitF(c)$, so it
ranges over at most $|\orbitF(c)|$ distinct outcomes.
(The orbit set is the \emph{support} of this observation; the orbit
map $c \mapsto \orbitF(c)$ is trapdoor-independent, but which outcome
the untrusted machine resolves to is what carries information about
$X$.)
...
\begin{proof}
The observation $\mathcal{V}_F(c)$ takes values in the finite set
$\orbitF(c)$, hence has at most $|\orbitF(c)|$ distinct outcomes, so
$H(\mathcal{V}_F(c)) \leq \log_2 |\orbitF(c)|$.
Therefore
\[
I(X; \mathcal{V}_F(c)) \;\leq\; H(\mathcal{V}_F(c)) \;\leq\; \log_2 |\orbitF(c)|,
\]
and since $H(X \mid \mathcal{V}_F(c)) = H(X) - I(X; \mathcal{V}_F(c))$,
the claim follows.
\end{proof}
