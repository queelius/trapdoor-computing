...
can go.
We realize cipher programs as expression-tree decompositions with
cipher-map cut points (exposed in Python via a
\texttt{@cipher\_node} decorator) and evaluate the cipher Boolean
algebra specialization on a 20~Newsgroups Boolean search task.
\end{abstract}
...
\item \textbf{Expression-tree realization} (\Cref{sec:realizing}).
  We decompose cipher programs into cipher-map pieces via cipher-node
  annotations on the program's expression tree.
  Each annotated node becomes a cipher map; unmarked nodes above
  them remain as plain operations on the cipher bytes.
...
single-trajectory bound of $T+1$ orbit elements per starting tape
configuration is straightforward but the leakage profile is severe:
...
The expression-tree realization concretizes these trade-offs at the
program level: cipher-node annotations on a Python program select the
cut points where cipher maps replace plaintext subtrees, while
unmarked nodes run as plain code on the cipher bytes.
