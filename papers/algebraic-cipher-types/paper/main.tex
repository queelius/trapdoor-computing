...
Cipher nodes cannot be chosen freely leaf-by-leaf.
...
be a cipher map too, since the untrusted machine cannot decode it.
Moreover a cipher map takes cipher values as inputs, so once a node is
ciphered its \emph{sibling} inputs must be cipher values as well.
The two forces together make the ciphered region upward-closed along
data-flow edges with the cut points as its lower frontier (the
combiner of \Cref{def:cipher-node} is exactly this region); only pure
...
root, a single cut whose result reaches the output through semantic
operations ciphers the entire tree (\Cref{fig:cipher-tree}).
Control flow is the sharp case.
In \texttt{if $e$ then $r_1$ else $r_2$} with $e$ ciphered, the test is
a cipher Boolean, and branching on it on the untrusted machine is
untrusted dispatch on the sum $\mathbf{1}+\mathbf{1}$, forbidden by
\Cref{thm:sum-impossibility}: it would leak the test.
The conditional must instead become a cipher multiplexer that
obliviously selects between $r_1$ and $r_2$
(\Cref{rem:oblivious-elim}), which forces \emph{both} branches to be
cipher values and both to be evaluated---exactly why the tracing pass
below computes every branch rather than the one the plaintext program
would have taken.
\end{remark}
...
its cipher output forces $<$ to become a cipher comparison, which pulls
in the sibling $y$; the cipher Boolean then forces \texttt{if} into a
cipher multiplexer, which pulls in both branches $p$ and $q$.
Double-circled shaded nodes are cipher nodes; the entire tree above the
cut is ciphered.}
...
and, by \Cref{thm:sum-impossibility}, leaks $t$ with advantage at least
$1-\delta$.
...
eliminator over the joint domain (\Cref{rem:oblivious-elim}).
\end{proposition}
...
valid encodings; this is the selector of \Cref{thm:sum-impossibility},
...
noise value (\Cref{prop:noise-unreliability}) that the multiplexer
discards.
...
product-encoding cost of \Cref{prop:product-tradeoff} resurfacing inside
control flow.
...
a cipher sum, and the dispatch obligation of \Cref{thm:sum-impossibility}
