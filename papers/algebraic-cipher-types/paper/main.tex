...
budget: the depth of a cipher computation determines how deeply its
orbit can grow.
We instantiate the algebra in a cipher Boolean type and evaluate it on
a 20~Newsgroups Boolean search task, and show how the same
bag-gated machinery supports partial filtering---returning a superset
of the relevant
documents while hiding which are actually relevant, trading precision
for confidentiality by a tunable amount.
\end{abstract}
...
\item \textbf{Instantiation and evaluation} (\Cref{sec:cipher-bool}).
  We instantiate the algebra in a cipher Boolean type with
  AND/OR/NOT and evaluate Boolean search on the 20~Newsgroups corpus,
  measuring precision, recall, and how the false-positive rate compounds
  through AND/OR chains (it tracks the Bernoulli prediction for OR but
  diverges sharply for AND).
  The same construction supports \emph{partial filtering}: returning a
  superset of the relevant documents while hiding which are relevant,
  a tunable precision-for-confidentiality trade governed by the orbit
  bound.
\end{enumerate}
...
We build cipher maps for chosen functions over a finite domain using the
batch (perfect-hash) construction of~\cite[Sec.~6]{towell2026cipher},
whose space ($-\log_2\varepsilon + \mu$ bits per element), correctness,
and representation-uniformity parameters we take as given and do not
re-derive.
This paper constructs cipher maps only to analyze how their algebraic
structure and composition leak; assembling many cipher maps into a
general program is a separate problem, treated
in~\cite{towell2026cipherprog}.
...
\begin{remark}[Algebra of the underlying trapdoor sets]
\label{rem:trapdoor-set-algebra}
The cipher Boolean type is the confidential lift of a simpler object: the
\emph{trapdoor set}, whose elements are one-way encodings
$\tau_k(v) = H(k \mathbin{\|} v)$ on which union, intersection, difference,
and symmetric difference act directly over the encodings.  The algebraic
structure of these sets is weaker than full Boolean algebra.  Over an
unbounded universe the finite trapdoor sets under
$(\cup, \cap, \setminus, \triangle, \emptyset)$ form a \emph{generalized}
Boolean algebra: a relatively complemented distributive lattice with least
element $\emptyset$ but no greatest element, equivalently a Boolean ring
without unit ($\triangle$ as addition, $\cap$ as multiplication).  They are
\emph{not} a Boolean algebra, because complement is absent:
$X \setminus A$ is infinite and has no finite trapdoor-set representation.
A finite universe restores the greatest element $\tau(X)$ and hence
complement $\tau(X) \setminus \tau(A)$, at the cost of materializing the
encoding of the entire universe.  The compact single-hash bit-vector
(Bloom) representation supplies a syntactic bitwise complement, but it does
not preserve set complement.  The cipher Boolean type of
\Cref{def:cipher-bool} is the confidential ($K > 1$, trusted-decode) lift
of this algebra; the deterministic ($K = 1$) case is its online
instantiation in the foundation paper~\cite{towell2026cipher}, where
membership is answered untrusted-side by equality of encodings and the
trapdoor set carries the leakage profile of deterministic and searchable
encryption.
\end{remark}
...
The product-encoding trade-off of \Cref{sec:products} is measurable.
We built a 7-function loan approval pipeline at three granularities
(\Cref{tab:granularity}): as one coarse cipher map over the full domain,
as $3$ grouped component maps, and as $7$ leaf maps, the finer two
composed by hand over the components' cipher outputs.
The coarse map uses the PHF backend; the component maps use a
seed-search backend, which occasionally fails to find a valid hash
assignment for a given component domain---which is why the intermediate
row reports $n = 2$ successful seeds rather than $5$.
...
construction.
The coarse map is roughly $400\times$ faster to build (PHF lookup vs.\
per-element seed search) and exposes zero intermediate cipher values.
Counter-intuitively, the finer constructions use \emph{less} total
space ($\approx 140$\,B vs.\ the coarse map's $694$\,B): each component
map's domain is smaller than the product domain of the whole function,
and the seed-search backend stores one $n$-bit seed per element.
The space-leakage trade-off is therefore not a uniform ``coarser is
larger''; the coarse map buys hidden intermediate values at the cost of
construction time and a build that requires the full input domain up
front, while the fine-grained maps are smaller and compose dynamically
but expose every intermediate cipher value (the correlation leakage of
\Cref{prop:product-tradeoff}).


%% --------------------------------------------------------------------
\subsection{Partial Filtering}
\label{sec:partial-filter}
%% --------------------------------------------------------------------

The Boolean search of \Cref{sec:evaluation} returns a per-document
verdict, so an untrusted machine observing the results learns which
documents match.
Hand composition buys a more interesting object: a cipher map that
returns a \emph{superset} of the relevant documents while hiding which
of them are actually relevant, trading precision for confidentiality by
a tunable amount.

Partition the corpus into $B$ bags of $m$ documents.
For a bag holding cipher document handles $d_1, \ldots, d_m$, let
$b_1, \ldots, b_m \in \cipher{\mathrm{Bool}}$ be the cipher Boolean
match results of a query against those documents
(\Cref{sec:cipher-bool}).
Construct a \emph{gate} in two steps.
First chain the cipher Boolean operations of \Cref{sec:cipher-bool} to
compute the cipher Boolean $\bigvee_i b_i$ (an $m$-ary cipher OR, or a
chain of binary ORs over the input domain $(T\cup F)^m$, cheaper than
the $O(2^m)$ flat table).
Then \emph{declassify} that single bit to a cleartext verdict
\[
\mathrm{gate}(b_1,\dots,b_m) =
\begin{cases}
\text{fire} & \text{if } \textstyle\bigvee_i b_i,\\
\text{idle} & \text{otherwise.}
\end{cases}
\]
The declassification is deliberate and is the only thing the scheme
reveals: the $m$ per-document match bits and every intermediate cipher
Boolean stay opaque, so the verdict says that the bag contains a match,
never which document.
When the gate fires, the untrusted machine returns the bag's $m$ handles
$d_1, \dots, d_m$; the trusted machine decodes them and applies the
exact per-document filter privately.

The untrusted machine therefore learns only \emph{which bags} contain a
match---one bit per bag, the coarse relevance vector---never which
document within a fired bag is the true match.
The relevant document is hidden among the $m$ in its bag: a $1$-in-$m$
anonymity set, with $m$ the tunable knob.
This is the orbit bound of \Cref{thm:confidentiality-bound} in
operational form: the adversary's per-bag observation ranges over a
two-element set (fired or idle), so it gains at most one bit per bag
about the latent relevance pattern, regardless of $m$.

The cost is precision.
If the $|R|$ relevant documents for a query fall in distinct bags,
partial filtering returns up to $m|R|$ documents of which $|R|$ are
relevant, so precision falls to $\approx 1/m$ while recall is preserved
(every relevant document's bag fires)---up to the cipher-OR noise floor,
since a noise value among the $m$ bits can flip a verdict, the same
effect that holds OR-query recall to $\approx 0.96$ in
\Cref{tab:boolean-search} (\Cref{prop:noise-unreliability}).
This is the same precision axis measured in \Cref{tab:boolean-search},
now a deliberate, tunable lever rather than an artifact of the
false-positive rate: larger bags hide the relevant document better and
return more chaff, smaller bags do the reverse.
Bags may be padded with decoy handles to raise $m$ without enlarging the
real corpus, a separate mechanism we do not pursue here.
...
\item \emph{Realizing whole programs.}
  Assembling cipher maps into a general program---placing cut points,
  propagating ciphering through control flow, and discharging the
  expose-versus-hide choice at each branch---is a substantial problem
  in its own right, with its own confidentiality and cost trade-offs.
  We take it up in a companion paper~\cite{towell2026cipherprog}.
\end{enumerate}
...
These results are about cipher \emph{types} and their composition,
established by constructing cipher maps for specific functions and
analyzing what they leak.
Assembling many cipher maps into a general program---deciding where to
place each one, propagating ciphering through control flow, and
discharging the expose-versus-hide choice at every branch---is a
distinct problem that we take up in a companion
paper~\cite{towell2026cipherprog}.
...
The encoding granularity spectrum is measurable: constructing a
function as one coarse cipher map is roughly $400\times$ faster to
build than as several fine-grained component maps, exposes no
intermediate cipher values, but consumes more space than the smaller
component maps; the trade-off is not a single axis but a
multi-dimensional choice over build time, total space, and intermediate
exposure.
