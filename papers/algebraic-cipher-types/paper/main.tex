...
(space $O(|Q|^2 \cdot |\Gamma|)$ for transition plus shift, leaks the
head-movement and state sequences), and evaluate the
...
\item \textbf{Cipher program realizations} (\Cref{sec:realizing}).
  ...
  with space $O(|Q|^2 \cdot |\Gamma|)$ at the cost of leaking the
  head-movement and state sequences.
...
$\tau_T : Q \times \Gamma \to Q \times \Gamma \times \{L, R, S\} \times Q$,
where the final $Q$-component of the output is a plaintext selector
$q_\mathrm{sel}$ for the new state.
...
current-head cell, obtaining a triple $(c', d, q_\mathrm{sel})$ where
$c'$ is a cipher value in $\cipher{Q \times \Gamma}$, $d \in \{L, R, S\}$
is the head direction in the clear, and $q_\mathrm{sel} \in Q$ is the
new-state selector in the clear.
...
and $\hat{\mathrm{set}}_{q_\mathrm{sel}} : \cipher{Q \times \Gamma} \to
\cipher{Q \times \Gamma}$, selected by the plaintext $q_\mathrm{sel}$,
places the head into the neighbouring cell at offset $d$.
The shift is a choice over a finite family of unary cipher maps,
one per $(q, d)$ pair, rather than a binary operation on adjacent
cells; this keeps every cipher map's domain at a single cell.
Two cleartext signals therefore leak per step: the head direction $d$
(necessary so the untrusted machine knows which neighbouring cell to
access) and the state selector $q_\mathrm{sel}$ (necessary so the
untrusted machine can choose the right $\hat{\mathrm{set}}_q$
instance without decoding $c'$).
\end{definition}
...
\begin{proposition}[Trajectory leakage]
\label{prop:head-determinism}
For a deterministic cipher TM with $\eta = 0$, the per-step plaintext
output sequence $(d_1, q_{\mathrm{sel},1}), (d_2, q_{\mathrm{sel},2}),
\ldots \in (\{L, R, S\} \times Q)^*$ is a deterministic function of
the input.
Two inputs that produce different trajectories are distinguishable by
the untrusted machine without the trapdoor.
\end{proposition}

\begin{proof}
With $\eta = 0$, $\hat{\tau}_T$ reproduces the plaintext Turing
machine's transitions exactly, so each $(d_i, q_{\mathrm{sel},i})$
matches the plaintext direction and next state at step $i$.
Distinct trajectories are directly observable by the untrusted machine.
\end{proof}
...
A cipher TM makes no such guarantee: tape contents are opaque, but the
sequence of head movements and state selectors is not.
This is weaker than ORAM but requires no $O(\log N)$ per-access
overhead.
Non-zero $\eta$ injects noise into the trajectory, but the noise is
seed-systematic rather than random, so it is unreliable as an
...
profile.
For a cipher TM, this is the trajectory: one $(d, q_\mathrm{sel})$
pair in the clear per step, plus the halting time.
...
Leakage: the trajectory $(d_i, q_{\mathrm{sel},i})$ for
$i = 1, \ldots, \ell$, revealing both the string length $\ell$ and the
DFA state sequence.
\emph{As an expression tree under the natural typed-chain
decomposition,} we place $\ell$ cut points $\hat{\tau}_0, \ldots,
\hat{\tau}_{\ell-1}$ with $\hat{\tau}_i : \cipher{Q_R}_i \times
...
The two realizations have the same orbit bound but different
construction-cost asymptotics: cipher TM is $O(|Q_R|^2 \cdot |\Sigma|)$
(\Cref{prop:cipher-tm-space}, including the auxiliary shift maps),
expression tree is $O(\ell \cdot |Q_R| \cdot |\Sigma|)$, and the
crossover sits near $\ell \approx |Q_R|$.
The qualitative difference is the \emph{leakage profile}: cipher TM
leaks the head-movement and state sequences (one $(d, q_\mathrm{sel})$
pair per step); the expression tree leaks the intermediate cipher
state values $c_0, \ldots, c_\ell$ flowing between cut points.
Cut-point placement therefore selects \emph{which structure leaks},
and the cost comparison is a separate axis that depends on the
relationship between $\ell$ and $|Q_R|$.
\end{example}
...
are exposed; a cipher Turing machine gives space $O(|Q|^2 \cdot |\Gamma|)$
at the cost of leaking the head-movement and state sequences.
Both are instances of a single cut-point pattern; their orbits are
bounded by complementary arguments (typed-chain reasoning for the
expression tree, deterministic-trajectory reasoning for the cipher
TM).
