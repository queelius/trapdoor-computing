...
SSE~\cite{curtmola2006searchable} enables keyword search on encrypted
documents with simulation-based security guarantees.
The Boolean search construction in \Cref{sec:cipher-bool} provides an
alternative with information-theoretic parameters: cipher sets with
cipher Boolean AND/OR/NOT, rather than encrypted indices with leakage
profiles.
Naveed et al.~\cite{naveed2015inference} demonstrated inference attacks
on property-preserving encryption; the cipher map model avoids
property preservation entirely (the untrusted machine cannot determine
the output type from the cipher value).
...
Given a cipher value $c$, operations $F$, and the orbit $\orbitF(c)$, the
\emph{candidate set} is the set of latent values consistent with the
observed orbit:
\[
\mathrm{Cand}_F(c) = \{ x \in X \mid \exists\, k \text{ s.t.\ }
\enc(x, k) = c \text{ and } \orbitF(\enc(x,k)) = \orbitF(c) \}.
\]
That is, $x$ is a candidate if encoding $x$ produces $c$ and the orbit
structure starting from $c$ matches what the adversary observes.
\end{definition}

\begin{definition}[Confidentiality]
\label{def:confidentiality}
The \emph{confidentiality} of a cipher value $c$ under operations $F$ is
the adversary's remaining uncertainty about the latent value:
\[
\mathrm{conf}_F(c) = 1 - \frac{|\mathrm{Cand}_F(c)|}{|X|}.
\]
When $\mathrm{conf}_F(c) = 1$, the adversary has learned nothing.
When $\mathrm{conf}_F(c) = 0$, the latent value is uniquely determined.
\end{definition}
...
\begin{proof}
Under $\delta \approx 0$, encodings are spread approximately uniformly
across $\B^n$.
The orbit $\orbitF(c)$ is a set of bit strings that the adversary can
compute but not decode.
In the worst case, every orbit element is a valid encoding of a distinct
latent value.
The adversary then knows the latent value is one of at most
$|\orbitF(c)|$ possibilities out of $|X|$ total.
Since $|X| \leq 2^n$ (at most $2^n$ distinct encodings), the fraction of
the cipher space explored is $|\orbitF(c)| / 2^n$, and the remaining
unexplored fraction bounds the confidentiality.

This bound is loose: it ignores the functional relationships between orbit
elements (e.g., the adversary knows that orbit element $c' = \fhat(c)$,
which constrains the possible latent values beyond mere set size).
...
A \emph{typed composition chain} of depth $k$ is a sequence of cipher
maps $\fhat_0, \fhat_1, \ldots, \fhat_{k-1}$ where
$\fhat_i : \cipher{A}_i \to \cipher{A}_{i+1}$ and the cipher spaces
$\cipher{A}_0, \cipher{A}_1, \ldots, \cipher{A}_k$ are \emph{distinct}
...
For a typed composition chain of depth $k$ with cipher spaces of sizes
$|\cipher{A}_0|, \ldots, |\cipher{A}_k|$, the orbit of any cipher value
$c \in \cipher{A}_0$ is bounded by
\[
|\orbitF(c)| \leq 1 + k,
\]
where each step adds at most one new cipher value (the output of $\fhat_i$
applied to the previous result).
The confidentiality satisfies
$\mathrm{conf}_F(c) \geq 1 - (1+k)/2^n$.
\end{proposition}

\begin{proof}
Starting from $c \in \cipher{A}_0$, the adversary computes
$c_1 = \fhat_0(c) \in \cipher{A}_1$,
$c_2 = \fhat_1(c_1) \in \cipher{A}_2$, and so on up to
$c_k = \fhat_{k-1}(c_{k-1}) \in \cipher{A}_k$.
No operation maps back into an earlier cipher space, so the orbit is
exactly $\{c, c_1, \ldots, c_k\}$ with $|\orbitF(c)| = 1 + k$.
The confidentiality bound follows from
\Cref{thm:confidentiality-bound}.
\end{proof}

\begin{example}[Depth-limited Boolean search]
\label{ex:typed-bool}
For Boolean search with maximum query depth 2 (e.g., $w_1 \land w_2$
but not $w_1 \land w_2 \land w_3$), define:
\begin{itemize}
...
\end{itemize}
The adversary can compose at most two AND operations.
The orbit from any cipher Boolean value has size at most 3.
Contrast with a single shared cipher Boolean space, where the
adversary can self-compose indefinitely and the orbit grows
without bound.
\end{example}
...
\emph{Boolean search at scale.}
Table~\ref{tab:boolean-search} shows precision and recall for
...
\emph{FPR compounding vs.\ Bernoulli model.}
The Bernoulli composition model~\cite{towell2026bernoulli} predicts
$\mathrm{FPR}_{\mathrm{AND},k} = p_T^k$ for $k$ independent AND
operations.
Empirically, the OR chain matches theory closely (ratio $\approx 0.9$
across chain lengths 1--5), but the AND chain diverges at depth $> 2$:
the empirical FPR exceeds the predicted $p_T^k$ by an order of
magnitude at $k = 3$.
The reason: the Bernoulli model assumes each test is an independent
draw, but the cipher Boolean AND is a deterministic cipher map.
After one AND, the result is a specific cipher Boolean value (not a
fresh random draw), and feeding it into the next AND produces a
deterministic output.
The independence assumption breaks at the cipher map level even though
it holds at the element level within a single cipher set.

\emph{Encoding granularity.}
...
confidentiality degradation as the untrusted machine applies operations.
