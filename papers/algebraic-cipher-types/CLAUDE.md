...
- **The paper has two distinct argument modes.** Sections 3 to 5 are theory (preliminaries, type-constructor trade-offs, orbit-closure framework). Section 6 is concrete instantiation and evaluation (cipher Boolean algebra, 20 Newsgroups search, partial filtering). Edits should respect those modes: don't mix experimental specifics into the theory. **Scope boundary (set 2026-06-03):** this paper constructs cipher maps of specific functions only to analyze leakage and composition trade-offs. It does NOT cover (a) how to construct a single cipher map (that is the foundation paper, `towell2026cipher` Sec 6, cited not re-derived), nor (b) how to realize an arbitrary program as a cipher-map composition (cut points, propagation, control-flow obliviousness, `@cipher_node` automatic rewriting). The latter was removed 2026-06-03 to a companion paper at `../cipher-program-construction/`; the removed §6 source is preserved in that repo. If you find yourself adding expression-tree/cut-point/branching-realization material here, it belongs in the companion.
