...
We implemented HBTS as a Python library (\texttt{cts}, approximately 200 lines of code) using only the standard library. The implementation provides four core modules:
...
The asymptotic complexity of HBTS operations is determined by the underlying hash computation and set operations:
...
All operations are dominated by hash computation cost. Since HBTS uses standard cryptographic hashing (SHA-256), individual hash evaluations complete in the sub-microsecond range on modern hardware, making HBTS orders of magnitude faster than homomorphic encryption or MPC-based alternatives for set operations.

\subsection{Security Evaluation}

The security of HBTS rests on the collision resistance and preimage resistance of the underlying hash function. For SHA-256, the expected collision probability is $2^{-256}$ per pair, providing a negligible false positive rate for practical set sizes.
...
We presented Hash-Based Trapdoor Sets (HBTS), a practical framework for privacy-preserving set operations that combines cryptographic hash functions with probabilistic data structures. By explicitly managing error rates and embracing approximation, HBTS achieves efficient performance---bounded by the cost of standard hash computations---while providing privacy bounded by hash collision probabilities.
...
\item \textbf{Differential privacy integration}: Combining HBTS with calibrated noise addition is a natural extension but not yet implemented
