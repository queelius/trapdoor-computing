...
\end{itemize}
We distinguish \emph{second-order} Bernoulli Booleans where $\fpr \neq \fnr$ (asymmetric errors, as in Bloom filters and hash-based membership) from \emph{first-order} where $\fpr = \fnr$ (symmetric errors). the trapdoor set produces second-order Bernoulli Booleans with $\fpr = 2^{-n}$ (hash collision) and $\fnr = 0$ (no false negatives).
\end{definition}
...
\end{itemize}
For the trapdoor set with $\fpr = 2^{-n}$ and $\fnr = 0$, the confusion matrix is nearly identity---high utility but query results are observable (the ``concession'' discussed in Section~\ref{sec:layered-privacy}).
\end{definition}
...
\label{rem:qif}
Theorem~\ref{thm:privacy} prevents recovering $x$ from $T_k(x)$, but says nothing about what the encodings leak \emph{collectively}. Under deterministic encoding ($K = 1$), an adversary observing the encoded set and a stream of queries learns the partition of values by equality and their frequencies---the leakage profile of deterministic and searchable symmetric encryption. The appropriate confidentiality measure is therefore quantitative information flow: the entropy of the observed encoding distribution relative to the prior, degraded precisely by the equality channel and improved by homophony ($K > 1$). Preimage hardness bounds non-recovery; it is not a confidentiality bound.
\end{remark}

\subsection{Approximate Algebraic Properties}

the trapdoor set exhibits approximate algebraic properties with explicit error bounds:
...
We implemented the trapdoor set as a Python library (\texttt{cts}, approximately 200 lines of code) using only the standard library. The implementation provides four core modules:

\begin{itemize}
\item \texttt{HashValue}: Fixed-size byte arrays with bitwise Boolean operations (AND, OR, XOR, NOT), providing the carrier for Bernoulli set operations.
\item \texttt{BernoulliBoolean}: Boolean values carrying explicit error rates $(\alpha, \beta)$, with confusion matrix computation, Bayesian posterior updates, and mutual information calculation.
\item \texttt{Trapdoor}/\texttt{TrapdoorFactory}: One-way transformation $T_k(v) = H(k \| v)$ using HMAC-SHA256, with key fingerprinting for compatibility checking across sets.
\item \texttt{BooleanSet}: Full Boolean algebra over sets of trapdoors, supporting union (\texttt{|}), intersection (\texttt{\&}), symmetric difference (\texttt{\^{}}), and set difference (\texttt{-}), with automatic error rate propagation and cardinality bound tracking through operations.
\end{itemize}

The library requires Python 3.10+ and has no external dependencies. All operations return \texttt{BernoulliBoolean} results with explicit error rates, and set operations propagate error bounds using the composition rules from Section~\ref{sec:design}. Implementation details including code listings are provided in Appendix~A.
...
This appendix provides implementation details for the \texttt{cts} Python library that accompanies this paper. The library is approximately 200 lines of code with no external dependencies beyond the Python standard library.

\subsection{Core Data Structures}

\subsubsection{Trapdoor Implementation}

The trapdoor factory creates one-way transformations using keyed SHA-256:

\begin{lstlisting}[caption={Trapdoor factory (trapdoor.py)}]
class TrapdoorFactory:
    def __init__(self, key=None, hash_size=32):
    ...
    def create(self, value):
        ...
        return Trapdoor(
            hash=HashValue(h.digest()[:self._hash_size]),
            key_fingerprint=self._fingerprint)
\end{lstlisting}

\subsubsection{Bernoulli Boolean Implementation}

\begin{lstlisting}[caption={Bernoulli Boolean with error tracking (approximate.py)}]
@dataclass(frozen=True, slots=True)
class BernoulliBoolean:
    """Boolean with explicit error rates."""
    value: bool
    alpha: float = 0.0  # FPR
    beta: float = 0.0   # FNR
    ...
    def confusion_matrix(self):
        return ((1 - self.alpha, self.alpha),
                (self.beta, 1 - self.beta))
\end{lstlisting}

\subsection{Error Composition Functions}

\begin{lstlisting}[caption={Error propagation rules (approximate.py)}]
def compose_and(a1, b1, a2, b2):
    """AND: FPR = product, FNR = inclusion-exclusion."""
    return (a1 * a2, b1 + b2 - b1 * b2)

def compose_or(a1, b1, a2, b2):
    """OR: FPR = inclusion-exclusion, FNR = product."""
    return (a1 + a2 - a1 * a2, b1 * b2)

def compose_not(alpha, beta):
    """NOT: rates swap."""
    return (beta, alpha)
\end{lstlisting}
...
\begin{lstlisting}[caption={Set operations with error propagation (boolean\_set.py)}]
class BooleanSet:
    def __or__(self, other):
        """Union with FPR propagation."""
        self._check_compatible(other)
        new_alpha, new_beta = compose_or(
            self.alpha, self.beta,
            other.alpha, other.beta)
        return BooleanSet(
            _elements=self._elements | other._elements,
            alpha=new_alpha, beta=new_beta,
            _low_k=max(self._low_k, other._low_k),
            _high_k=self._high_k + other._high_k)

    def __and__(self, other):
        """Intersection with FPR propagation."""
        self._check_compatible(other)
