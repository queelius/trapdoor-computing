...
HBTS provides \emph{trapdoor representation}\footnote{We use ``trapdoor'' to mean a one-way encoding keyed by a secret, not a trapdoor permutation in the standard cryptographic sense. The hash key $k$ enables creation of encodings but does not permit recovery of inputs---there is no efficient inversion.} through one-way hash transformations---the underlying data cannot be recovered from hash values. Operations return \emph{Bernoulli Booleans}: plaintext approximate results with explicit error rates (false positive rate $\alpha$, false negative rate $\beta$). This layered model accepts observable query results in exchange for practical efficiency, making it suitable for applications where query patterns are not sensitive or results are shared.
...
Since HBTS operations reduce to standard hash computations and set operations, they are orders of magnitude faster than homomorphic encryption or MPC approaches. The framework provides false positive rates bounded by hash collision probability (e.g., $2^{-256}$ for SHA-256), with privacy guarantees that depend on input entropy and threat model assumptions. We provide a reference Python implementation and validate HBTS through applications to private set intersection, secure deduplication, and federated learning aggregation, showing practical applicability where approximate results with explicit error bounds are acceptable.
\end{abstract}
...
\item \textbf{Reference Implementation}: We provide a Python reference implementation demonstrating that combining cryptographic hashing with probabilistic data structures yields practical privacy-preserving set operations using only standard-library primitives.
