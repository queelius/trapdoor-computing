...
on which spending modest length overhead off the Shannon corner buys up to
a $42\times$ reduction in leakage.  A frequency-analysis
...
and minimized (best for the defender) by concentrating $\alpha$ on
the heaviest values.  Concentrated, Huffman-style partitions are
therefore preferred at every $t \geq 1$; there is no codec-choice
regime change between $t = 1$ and $t \geq 2$.
\end{corollary}
