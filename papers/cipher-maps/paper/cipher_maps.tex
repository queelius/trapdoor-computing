...
corollary (Corollary~\ref{cor:t-geometry}) is verified empirically:
at $t = 5$, the uniform Dense partition produces attacker accuracy
$\approx 0.9999$ (error rate $\approx 1.2 \times 10^{-4}$) while the
Shannon-optimal Huffman partition produces attacker accuracy
$\approx 0.984$ (error rate $\approx 0.016$).  Huffman therefore
defends $\approx 132\times$ better than uniform at $t = 5$ in this
configuration, consistent with the geometry: Huffman concentrates
the partition mass on the heavy mode, raising $\sum_y \alpha(y)^t$
and lowering attacker accuracy.
