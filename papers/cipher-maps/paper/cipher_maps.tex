...
gives $\TV(Q, \Uniform(\im(\enc))) \leq |X|/N$, which vanishes as the
budget $c$ (and hence $N \geq c$) grows.
\end{proposition}
...
$r(x) \in [0, 1)$, so $N = c + R$ where $R = \sum_x r(x) < |X|$ and
$K(x)/N - D(x) = (r(x) - R\,D(x))/N$.  Hence
\[
\TV\bigl(Q, \Uniform(\im(\enc))\bigr)
  = \frac{1}{2N} \sum_{x \in X} |r(x) - R\,D(x)|
  \leq \frac{1}{2N}\Bigl(\sum_x r(x) + R \sum_x D(x)\Bigr)
  = \frac{R}{N} < \frac{|X|}{N}. \qedhere
\]
\end{proof}
