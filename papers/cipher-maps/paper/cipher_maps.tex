...
(bounded here via the Fannes--Audenaert inequality, and developed as a
confidentiality measure in companion work~\cite{towell2026maxconf}).  The
...
predicates trace a frontier between expected codeword length $L$ and
TV-leakage: the length-optimal (Shannon) corner and the TV-optimal
corner are distinct under integer codeword constraints, and on
heavy-tailed distributions the homophonic multiplicity lever drives
TV-leakage down by an order of magnitude (a $42\times$ reduction in our
example) at a storage rather than a length cost.  Composition is
...
seeds) characterizes multi-instance leakage in closed form.  Both
single-instance and multi-instance optima favor concentrated
(Huffman-style) partitions; when multi-instance leakage exceeds
deployment tolerance, the appropriate defense is randomized encoding
($K(x) > 1$) rather than codec retuning.  We formalize the
...
predicates and quantifying the homophonic multiplicity lever, which
flattens the cipher value distribution by an order of magnitude
($42\times$ in TV on heavy-tailed value distributions).

\paragraph{Bernoulli error model.}
The error framework underlying cipher maps is the Bernoulli
model~\cite{bernoulli2026sets}.  Two axioms (element-wise independence and
...
(Proposition~\ref{prop:confidentiality}); the entropy ratio is developed
as a confidentiality measure in companion work~\cite{towell2026maxconf}.  This puts
...
The representation uniformity property is a modern instance of
homophonic substitution~\cite{simmons1979symmetric}, in which frequent
plaintext symbols are assigned multiple ciphertext equivalents to
flatten frequency distributions.  Writing $D(x)$ for the prior
probability that the trusted machine encodes latent value $x$ and
$K(x) \geq 1$ for the number of cipher representations assigned to $x$
(formal definitions in \S\ref{sec:abstraction} and
\S\ref{sec:properties}), the standard Simmons prescription
$K(x) \propto D(x)$ assigns more representations to frequent values so
that each cipher representation is queried with equal expected
frequency.  The connection is direct: representation uniformity
generalizes homophonic substitution from substitution ciphers to
...
written $\cipherS{f}{s} : \cipherS{X}{s} \to \cipherS{Y}{s}$; this is
exactly the total function $\fhat$ of Definition~\ref{def:cipher-map},
so $\cipherS{f}{s} = \fhat$.  Applied to the function type itself,
$\cipherS{X \to Y}{s}$ denotes the cipher space whose inhabitants are
precisely the maps $\cipherS{X}{s} \to \cipherS{Y}{s}$, and the cipher
map $\cipherS{f}{s}$ is one such inhabitant; thus $\cipher{\cdot}$
acting on the morphism $f$ and on the object $X \to Y$ agree (the
\emph{cipher exponential}, \cite[Prop.~4.3]{towell2026algebraic}).
Under the
...
$K(x) \propto D(x)$ equalizes frequencies.  This yields
...
Let $D : X \to [0, 1]$ be the prior probability distribution from which
the trusted machine draws latent values for encoding (the query
distribution).  Let $K(x) \geq 1$ be the multiplicity assigned to $x$
(Definition~\ref{def:cipher-map}: the number of distinct cipher
representations of $x$).  Define the induced distribution on cipher
values observed by the untrusted machine:
\begin{equation}
Q(c) = \sum_{x \in X} D(x) \cdot \frac{|\{k : \enc(x,k) = c\}|}{K(x)}.
\end{equation}
The cipher map has \emph{$\delta$-representation uniformity} if
\begin{equation}
\TV(Q,\, \Uniform(\im(\enc))) \leq \delta,
\end{equation}
where $\TV$ is the total variation distance between probability
distributions on $\{0,1\}^n$, given by
\begin{equation}
\TV(P, Q) \;=\; \tfrac{1}{2} \!\!\!\! \sum_{c \in \{0,1\}^n} \!\!\!\! |P(c) - Q(c)|
  \;=\; \!\! \sup_{A \subseteq \{0,1\}^n} \!\!\!\! |P(A) - Q(A)|.
\end{equation}
\end{definition}

The reference is the uniform distribution on the populated support
$\im(\enc)$, not the ambient $\{0,1\}^n$, which is unsatisfiable for sparse
images.  Total variation distance has the operational interpretation that no
statistical test can distinguish $Q$ from uniform with advantage
exceeding $\delta$.  To achieve small $\delta$, assign
$K(x) \propto D(x)$ encodings per value (the classical Simmons
homophonic prescription~\cite{simmons1979symmetric}): frequent values
get more representations, so the per-representation query frequency
$D(x) / K(x)$ is constant across~$x$, and $Q$ is flat across
cipher values.  The following proposition makes this precise: the
representation-uniformity gap is exactly the total variation distance
between the query distribution $D$ and the normalized integer
allocation $K/N$.

\begin{proposition}[Homophonic allocation]
\label{prop:homophonic}
Suppose $\enc$ is injective, so the image $\im(\enc)$ contains exactly
$N := \sum_{x \in X} K(x)$ cipher values, and let $A(x) := K(x)/N$
denote the normalized allocation (a probability distribution on $X$).
Then the induced cipher value distribution $Q$ of
Definition~\ref{def:rep-uniformity} satisfies
\begin{equation}
\TV\bigl(Q,\, \Uniform(\im(\enc))\bigr)
  \;=\; \tfrac{1}{2} \sum_{x \in X} \Bigl| D(x) - \tfrac{K(x)}{N} \Bigr|
  \;=\; \TV(D, A).
\end{equation}
In particular, the Simmons allocation $K(x) = \lceil c\,D(x) \rceil$
gives $\TV(Q, \Uniform(\im(\enc))) < (|X|-1)/N$, which vanishes as the
budget $c$ (and hence $N \geq c$) grows.  The constant $|X|-1$ is tight:
distributions concentrating mass on one value while the other $|X|-1$
values sit just above the rounding boundary approach it.
\end{proposition}

\begin{proof}
By injectivity, $\im(\enc)$ partitions into $|X|$ groups
$\{\enc(x,0), \ldots, \enc(x,K(x){-}1)\}$, one per $x$, of total size
$N$.  For $c = \enc(x,k)$ we have $Q(c) = D(x)/K(x)$ (draw $x$ with
probability $D(x)$, then $k$ uniform over its $K(x)$ indices) and
$\Uniform(\im(\enc))$ assigns mass $1/N$.  Summing the
absolute differences group by group,
\[
\sum_{c \in \im(\enc)} \bigl|Q(c) - \tfrac1N\bigr|
  = \sum_{x \in X} K(x)\,\Bigl|\tfrac{D(x)}{K(x)} - \tfrac1N\Bigr|
  = \sum_{x \in X} \Bigl| D(x) - \tfrac{K(x)}{N} \Bigr|,
\]
and halving gives the first equality; the second is the definition of
$\TV(D, A)$, valid since $\sum_x A(x) = N/N = 1$.  For the Simmons
allocation write $K(x) = c\,D(x) + r(x)$ with rounding excess
$r(x) \in [0, 1)$, so $N = c + R$ where $R = \sum_x r(x)$ and
$K(x)/N - D(x) = (r(x) - R\,D(x))/N$.  The deviations
$r(x) - R\,D(x)$ sum to zero, since $\sum_x r(x) = R$ and
$\sum_x R\,D(x) = R$; hence their positive and negative parts are
equal and
\[
\TV\bigl(Q, \Uniform(\im(\enc))\bigr)
  = \frac{1}{N} \sum_{x \in P} \bigl(r(x) - R\,D(x)\bigr),
  \qquad P := \{x : r(x) > R\,D(x)\}.
\]
Because the deviations sum to zero they cannot all be positive, so $P$
is a proper subset of $X$ and $|P| \leq |X| - 1$.  Using
$R\,D(x) \geq 0$ and $r(x) < 1$,
\[
\TV\bigl(Q, \Uniform(\im(\enc))\bigr)
  \leq \frac{1}{N} \sum_{x \in P} r(x)
  < \frac{|P|}{N} \leq \frac{|X|-1}{N}. \qedhere
\]
\end{proof}

The proposition bounds the gap to uniformity \emph{within the image}
$\im(\enc)$; the residual gap to $\Uniform(\{0,1\}^n)$ in
Definition~\ref{def:rep-uniformity} is the noise region outside the
image, populated by totality (Property~1) and noise injection
(\S\ref{sec:trust-model}).
...
distribution $\mathrm{Uniform}(\B^n)$ (entropy $n$), and using
$\log_2(2^n - 1) \le n$, gives
$H(Q) \geq n(1 - \delta) - h_2(\delta)$, and dividing by $n$ yields
\eqref{eq:fannes-bridge}.  The tightness of this bound is analyzed in
...
Table~\ref{tab:guarantees} collects these bounds against the
honest-but-curious $U$.  Its last row is the honest limitation:
marginal $\delta$-uniformity per cipher map does not bound joint
correlations across multiple cipher maps sharing inputs.  The compositional leakage analysis in
...
\approx 1.44 n$ term (a constant $\approx 1.44$ bits per element):
dropping this positive term only weakens the lower bound, and the
matching achievability construction (Theorem~\ref{thm:space-optimal})
absorbs it asymptotically.  The noise-decode probability
...
\paragraph{Shannon-optimal allocation and the leakage frontier.}
Setting $\alpha(y) \propto \Pr[f(X)=y]$ achieves two goals at once
(\Cref{fig:acceptance-partition}).
...
distribution.  Second, it aligns noise with real traffic: noise queries
(random hash values) hit acceptance set $A(y)$ with probability
$\alpha(y) \propto p_y$---the same profile as real queries---so an
adversary cannot use the output profile to tell whether a result came from
a real query or from injected noise.  These two goals are the same
allocation, not a coincidence: the length-optimal code is exactly the one
whose acceptance profile matches the real output profile.

This duality is between \emph{space} and \emph{real-versus-noise
matching}, and it should not be mistaken for minimizing \emph{value-side}
leakage.  The latter is the representation-uniformity parameter $\delta$
of Property~2---the total-variation distance of the observed cipher value
distribution from uniform---and driving it down is a \emph{separate}
objective, achieved not by the length-optimal code but by the multiplicity
lever ($K(x) > 1$, the homophonic mechanism of Property~2), which spreads
each value over several codewords at the cost of extra length.  The
length-optimal (Shannon) corner and the $\delta$-optimal corner coincide
only when $p_y$ is dyadic; in general they are distinct points on a
$(\mathrm{TV}, L)$ Pareto frontier (\S\ref{subsec:eval-codec-security}),
on which spending modest length overhead off the Shannon corner gives
only a small ($\approx 2\times$) TV reduction; the order-of-magnitude
reductions ($42\times$ on the same heavy-tailed example) come instead
from the orthogonal multiplicity lever ($K(x) > 1$), which trades
storage for a flatter $Q$.  A frequency-analysis
adversary~\cite{naveed2015inference} therefore constrains the choice of
operating point: the length-optimal code is not automatically the most
confidential one.
...
The batch construction encodes
\begin{equation}
(1-\eta)\bigl(-\log_2 \varepsilon + \mu\bigr) \text{ bits per element}
\end{equation}
of information, where $\mu = H(Y)$ is the mean encoding length.  This is
the information content of the $(1-\eta)n$ correctly encoded elements;
physical storage is $-\log_2 \varepsilon + \mu$ per element regardless of
$\eta$ (Remark~\ref{rem:physical-storage}).  When $\eta = 0$ the two
coincide and match the lower bound of Theorem~\ref{thm:lower-bound}.
\end{theorem}
...
When $\eta = 0$, the batch construction's storage is
$-\log_2 \varepsilon + H(Y)$ bits per element
(Remark~\ref{rem:physical-storage}), matching the information-theoretic
lower bound of Theorem~\ref{thm:lower-bound}.
\end{corollary}
...
\end{itemize}
The expected number of seeds tried is $1/p$ (fresh seeds $\ell$ make the
trials i.i.d.\ with success probability $p$ under the random oracle
model).
\end{proposition}
...
\label{prop:bucketed}
With $k$ buckets of expected size $m/k$ and $\eta = 0$, the construction
time under the equal-load approximation is
\begin{equation}
T(k) = k \cdot \left(\frac{1}{\bar{\alpha}}\right)^{m/k},
\end{equation}
where $\bar{\alpha} = \bigl(\prod_i \alpha(y_i)\bigr)^{1/m}$ is the
geometric mean acceptance probability.  Random bucket sizes make this a
lower bound on the true expectation (Jensen); multinomial concentration
makes it tight for $m/k$ large.
\end{proposition}
...
output distribution $(p_y)_{y \in Y}$ is
$-\log_2 \varepsilon + H(Y)$
bits up to the one-bit prefix-coding gap, where
$H(Y) = -\sum_y p_y \log_2 p_y$.
\end{proposition}
...
Shannon-optimal prefix-free coding: by Shannon's source coding
theorem~\cite{shannon1948mathematical} it satisfies $H(Y) \le L < H(Y)+1$,
with equality when $(p_y)$ is dyadic (and approached for general $(p_y)$
by block coding).  These two components are
...
($\Pr[A \cap B] \geq \eta_f \eta_g$), making the bound
...
(so $\sum_{y} \alpha_i(y) = \varepsilon_i \le 1$) and
$\Pr[\dec_i(\fhat_i(c_i)) = \bot] = 1 - \varepsilon_i$, with
...
A replicated sweep over five acceptance predicates and
$t \in \{1, 2, 3, 4, 5\}$, with $10$ replicates per cell at
$|S| = 2000$ and $|Y| = 8$, finds $23$ of $25$ cells with the
predicted accuracy inside the pooled Wilson $95\%$ confidence
interval; mean absolute gap between empirical and predicted is
$4 \times 10^{-4}$ (max $1.7 \times 10^{-3}$).  See
\S\ref{subsec:eval-codec-security} for the full sweep.
...
$(\max_y \alpha(y))^t$ as $t$ grows.  At fixed $\varepsilon = \sum_y
\alpha(y)$, this sum is minimized when $\alpha$ is uniform
($\alpha(y) = \varepsilon / |Y|$) and maximized when $\alpha$ is
concentrated on a single value (e.g., $\alpha(y^*) = \varepsilon$ for
some $y^*$ with $\alpha(y) = 0$ elsewhere).  Since the attacker's
prediction accuracy is $1 - \tfrac{1}{2} \sum_y \alpha(y)^t$,
\emph{smaller} $\sum_y \alpha(y)^t$ means \emph{higher} attacker
accuracy, i.e., \emph{worse} defense.

\begin{corollary}[Coincidence-oracle accuracy and partition geometry]
\label{cor:t-geometry}
Among acceptance partitions with fixed $\varepsilon$, the
coincidence-oracle attacker's accuracy
$1 - \tfrac{1}{2} \sum_y \alpha(y)^t$ is maximized (worst for the
defender) by the uniform allocation $\alpha(y) = \varepsilon / |Y|$
and minimized (best for the defender), by Schur-convexity of
$\sum_y \alpha(y)^t$, by concentrating $\alpha$ on a single value
(Huffman-style partitions approximate this extreme).  Concentrated
partitions are therefore preferred at every $t \geq 1$; there is no
codec-choice regime change between $t = 1$ and $t \geq 2$.
\end{corollary}

\paragraph{Numerical illustration.}
For $|Y| = 8$ with a Zipf $p_y$ (skew parameter $s = 1.2$), the
Shannon-optimal Huffman partition gives $\max_y \alpha(y) \approx
0.5$ (the dominant value claims half the codespace), while uniform
gives $\max_y \alpha(y) = 1/8 = 0.125$.  At $t = 5$: Huffman has
$\sum_y \alpha(y)^5 \approx 0.031$ and attacker accuracy $\approx
0.984$ (attacker error rate $\approx 0.016$); uniform has $\sum_y
\alpha(y)^5 = 8 \cdot (1/8)^5 \approx 2.4 \times 10^{-4}$ and
attacker accuracy $\approx 0.9999$ (attacker error rate
$\approx 1.2 \times 10^{-4}$).  Huffman therefore defends
$\approx 132 \times$ better than uniform in this configuration
(error rate ratio $0.016 / 0.00012$).  The single-instance Le Cam
mechanism and the multi-instance coincidence mechanism both favor
the same concentrated partition.

\paragraph{Implication for deployment.}
Both single-instance and multi-instance pressures favor Shannon-optimal
(Huffman-style) concentrated partitions.  The randomized-encoding
defense below addresses the multi-instance regime by raising
multiplicity $K(x) > 1$ rather than by retuning the codec; it is the
appropriate response when shared-$f$ multi-instance composition is
unavoidable.
...
\item[Decode-coincidence attacker (public-codec threat).]  Observes
    ...
    \]
    and predicts ``in-domain'' iff all $t$ decoded values agree on a
    common value in $Y$.  Requires knowledge of the per-instance
...
$K(x)$ equals the codespace class size $2^{n - \ell_{f(x)}}$).  We take
the $t$ instances to share a common acceptance geometry
($A_i(y) = A(y)$, the homogeneous-codec setting); under heterogeneous
regions the member-coincidence probability below becomes an upper bound.
Then under the random oracle model with independent seeds:
...
statistically independent across instances; by the common-geometry
assumption the pattern fixed by instance~$1$ lies in every $A_i(y)$.
The probability that all $t$ patterns coincide is the probability that
instances $2, 3, \ldots, t$ each match it: $\prod_{i=2}^{t} 1/|A_i(y)|$.
...
\item The defense incurs space cost only when the acceptance region
    has codespace slack, $|A(y)| > 1$ (equivalently, multiplicity
    $K(x) > 1$ is achievable).  The Dense partition
    ($\alpha(y) = 1/|Y|$, saturated) has $|A(y)| = 1$ for all $y$ and
    gets no benefit from randomized encoding.
\item The decode-coincidence accuracy formula
    \eqref{eq:coincidence-homogeneous} still applies under the
    public-codec threat; the structural defenses in that regime are
    concentrating $\alpha$ on the heavy modes
    (Corollary~\ref{cor:t-geometry} shows Huffman-style partitions
    are optimal at every $t$) and the encode-different-$f$-per-instance
...
the Bernoulli map algebra~\cite{bernoulli2026maps} and the algebraic-type
model~\cite{bernoulli2026datatype}.
...
\emph{trapdoor Boolean algebra}, is the online instantiation of the cipher
map framework, and its confidentiality is governed by the same homophony
dial $K(x)$ as the batch case.  At $K = 1$ (deterministic encoding) the
construction occupies the weakest corner of the design space: set elements
are deterministic one-way encodings, membership is answered on the untrusted
side by equality of encodings, and the result is a plaintext Boolean.  This
exposes both the equality predicate among encodings and the query answer
itself---the leakage profile of deterministic and searchable encryption, the
very regime the cipher map framework exists to improve upon.  Raising
$K > 1$ closes the equality channel (Property~2, representation uniformity)
and returns membership to the opaque, trusted-decode regime: the untrusted
machine then computes Boolean operations over opaque hashes without learning
any truth value.  The finite trapdoor sets form a generalized Boolean
algebra (a Boolean ring without unit) rather than a Boolean algebra, since
the complement of a finite set is not finitely representable;
\cite{towell2026algebraic} gives the algebraic characterization.  The batch
and online strategies offer complementary trade-offs: batch achieves
information-theoretically optimal space with tunable correctness; online
enables incremental construction and algebraic composition at the cost of
space exponential in set size.
...
value-only attacker's accuracy of $0.572$ (itself at the Le~Cam bound)
at the same operating point.  When
...
\paragraph{Bloom-filter baseline at matched FPR.}
To anchor the space cost of the trapdoor structure, we compare
per-document cipher set indexes against optimal Bloom filters at
matched per-document false-positive rate, on the 20 Newsgroups
training split ($11{,}004$ documents after removing empties).  For
$m \in \{1000, 5000, 11004\}$ documents and target
$\varepsilon \in \{0.01, 0.05, 0.10\}$, mean per-document
bits-per-element are reported in Table~\ref{tab:bloom-baseline}.
...
\begin{tabular}{lrrrr}
\toprule
$\varepsilon$ & Bloom (bits/elem) & Cipher (bits/elem) & Ratio & Cipher FPR \\
\midrule
0.01 & 9.60 & 30.7 & 3.20 & 0.0078 \\
0.05 & 6.25 & 28.7 & 4.59 & 0.0312 \\
0.10 & 4.81 & 27.7 & 5.76 & 0.0625 \\
\bottomrule
\end{tabular}
\caption{Per-document cipher set vs.\ optimal Bloom filter at matched
FPR on 20 Newsgroups.  Numbers averaged across the three $m$ values
(differences across $m$ are within $0.04$ bits/element for cipher and
within $0.02$ for Bloom).  Cipher empirical FPR matches the theoretical
$1/2^{n_{\mathrm{bits}}}$ target to within $0.0001$; Bloom matches
the asymptotic optimal-$k$ prediction to within $30\%$.}
\label{tab:bloom-baseline}
\end{table}

The 3-6$\times$ overhead is dominated by phobic's per-PHF fixed-cost
overhead at the per-document scale (mean vocabulary $\sim 98$
tokens).  Decomposing the cipher cost as $\mathrm{PHF}_{\mathrm{bits}}
+ n_{\mathrm{bits}}$: the PHF contribution is roughly $23.6$
bits/key (constant across $m$ and $\varepsilon$), while the
fingerprint contribution scales as $\lceil \log_2(1/\varepsilon) \rceil$.
Bloom shrinks with $\varepsilon$ (per the
$1.44 \cdot \log_2(1/\varepsilon)$ formula) while cipher is roughly
flat, so the ratio widens at large $\varepsilon$.  The
\emph{theoretical} lower-bound ratio (replacing the per-PHF
$23.6$ overhead with the asymptotic $\sim 1.8$ bits/key that phobic
approaches at scale) is $1.07$: at single-PHF deployment over all
$(\mathrm{doc}, \mathrm{token})$ pairs (~$1.8 \times 10^6$ keys),
cipher set space would be within $7\%$ of Bloom.  The $4.5\times$
overhead reported here is the price of per-document trapdoor
isolation; deployments that can share a single PHF across documents
recover near-Bloom space at the cost of giving up document-level
independence.

\paragraph{Reproducibility.}
The benchmark scripts and inputs are published with the reference
library; a single command reproduces the boolean-search numbers above,
and the Bloom-filter comparison (script, raw results, and discussion)
ships alongside them.  The numbers above are from one run per cell;
variance characterization across replicates and against SSE baselines
at matched operating points is work in progress
...
= \eta$, a deliberately injected symmetric error distinct from the
one-sided correctness parameter of Definition~\ref{def:correctness}),
and let $\pi = \Pr[f(x) = 1]$ be the prior.  If the
...
\begin{itemize}
\item Single-PHF deployment over all $(\mathrm{doc}, \mathrm{token})$
    pairs to verify the theoretical ratio-1.07 prediction at scale
    (per the Bloom-baseline analysis in
    \S\ref{subsec:eval-20ng}: the per-document deployment incurs a
    $4.5\times$ overhead vs.\ Bloom that is dominated by phobic's
    per-PHF fixed cost; a single-PHF deployment over $\sim 10^6$ keys
    should recover near-Bloom space).
...
Zipf with skew $s = 1.2$.  Cipher value space $\{0,1\}^n$ with $n$
between $3$ and $5$ bits depending on codec; specific codec sets
differ per analysis paragraph below.  The \emph{single-instance
Le~Cam tightness study} uses five codecs at $n \in \{4, 5\}$:
$\mathrm{Huffman}(n{=}5)$, $\mathrm{Huffman}(n{=}4)$,
$\mathrm{Dense}(n{=}4)$, $\mathrm{Padded}(n{=}4, \text{default}{=}y_7)$,
and $\mathrm{AntiHuffman}(n{=}5)$ (worst-case codec that swaps the
value-to-codeword-length pairing).  The \emph{replicated coincidence-oracle
sweep} (E3) uses a different five-codec set chosen for multi-instance
analysis: $\mathrm{Dense}(M{=}3)$, $\mathrm{Dense}(M{=}4)$,
$\mathrm{Padded}(M{=}4, v_0)$, $\mathrm{Huffman}(M{=}4, p_f)$,
$\mathrm{AntiHuffman}(M{=}4)$.  The \emph{randomized-encoding sweep}
(E4) uses Huffman codecs at $n \in \{4, 5, 6, 7, 8\}$ plus
$\mathrm{Dense}(n{=}3)$.  Three attacker classes are evaluated in the
Le Cam study: a Bayes likelihood-ratio classifier over the decoded
output, vanilla logistic regression on one-hot value features, and
1-nearest-neighbor.  Train and test sets are 5000 each at balanced
class prior; AUC is Mann-Whitney $U$ with tie correction.  Le Cam
results are single-run; E3 uses $10$ replicates per cell, E4 uses
...
Huffman ($n = 4$)        & 0.121 & 0.561 & 0.575 \\
Dense ($n = 4$)          & 0.218 & 0.609 & 0.607 \\
...
\item For heavy-tailed $p_y = (0.9, 0.0143, \ldots)$ at $n = 6$ bits,
    Huffman has $\mathrm{TV} = 0.40$.  Within single-codeword length
    assignments the gain is modest: the TV-min assignment reaches only
    $\mathrm{TV} = 0.20$ (a $1.9\times$ reduction) at $17\%$ overhead in
    expected length, because the heavy mode's acceptance probability is
    capped at $2^{-1}$, flooring its TV contribution.  Order-of-magnitude
    flattening instead comes from the multiplicity lever of
    Proposition~\ref{prop:homophonic} (assigning $K(y) \propto p_y$
    codewords per value, $\alpha(y) = K(y)/2^n$): here
    $K = (57, 1, \ldots, 1)$ gives $\mathrm{TV} = 0.009$, a $42\times$
    reduction, at a storage cost (the cipher space holds
    $N = \sum_y K(y)$ codewords) rather than a length cost.
\end{itemize}
...
cipher map instances under independent seeds.  A replicated
empirical sweep over $5$ codecs at $t \in \{1, 2, 3, 4, 5\}$
($10$ replicates per cell, $10{,}000$ trials per replicate)
finds $23$ of $25$ cells with the theoretical prediction inside
the Wilson $95\%$ CI from pooled $100{,}000$ trials; mean absolute
gap between empirical mean and prediction is
$4 \times 10^{-4}$ (max $1.7 \times 10^{-3}$).  The two cells
outside CI are both at high $t$ near accuracy $1$
(Dense($M{=}3$) $t{=}4$ predicted $0.9990$ vs.\ CI upper $0.9990$
with gap $-2 \times 10^{-4}$; Huffman($M{=}4$) $t{=}5$ predicted
$0.9844$ vs.\ CI $[0.9821, 0.9837]$ with gap $-1.4 \times 10^{-3}$),
where the Wilson CI half-width shrinks below the empirical-vs-prediction
sampling noise scale.  Both deviations are well below the
$0.01$ threshold for paper-claim impact; the theorem is empirically
tight to within sampling noise across the full sweep.

The geometric corollary (Corollary~\ref{cor:t-geometry}) is
verified empirically: at $t = 5$, the uniform Dense partition
produces attacker accuracy $\approx 0.9999$ (error rate
$\approx 1.2 \times 10^{-4}$) while the Shannon-optimal Huffman
partition produces attacker accuracy $\approx 0.984$ (error rate
$\approx 0.016$).  Huffman therefore defends $\approx 132\times$
better than uniform at $t = 5$ in this configuration, consistent
with the geometry: Huffman concentrates the partition mass on the
heavy mode, raising $\sum_y \alpha(y)^t$ and lowering attacker
accuracy.

There is no single-instance versus multi-instance trade-off in
codec choice: Huffman dominates at every $t$, both under the
single-instance Le Cam bound and under the multi-instance
coincidence-oracle bound.  Practical deployments publishing $t \geq 2$
cipher maps for the same latent function should retain the
Shannon-optimal partition and address multi-instance leakage via
randomized encoding (Proposition~\ref{prop:rand-encoding}).

\paragraph{Randomized encoding defense, $K$ sweep.}
The randomized-encoding defense (Proposition~\ref{prop:rand-encoding})
is validated empirically across a Huffman codespace-class sweep
$M \in \{4, 5, 6, 7, 8\}$ at $|S| = 1000$, $t \in \{2, 3, 4\}$, $5$
replicates per cell.  Three sub-hypotheses corresponding to
Proposition~\ref{prop:rand-encoding} were verified:

\begin{description}
\item[H1, pattern-coincidence decay with $K$.]
The member pattern-coincidence rate under randomized encoding
decreases monotonically with the codespace class size $|A(y)| =
2^{M - \ell_y}$, across all $t$.  At $t = 4$: Huffman pattern TPR
drops from $0.4156$ at $M = 4$ to $0.1058$ at $M = 5$, $0.0116$ at
$M = 6$, $0.0018$ at $M = 7$, and $0.0002$ at $M = 8$.  Empirical
values match the closed form
$\sum_v p_f(v) \cdot 2^{-(M - \ell_v)(t - 1)}$ of
Remark~\ref{rem:huffman-member-coincidence} to within $0.02$ across
all $15$ Huffman cells ($13$ of $15$ within $0.01$; two
$\mathrm{Huffman}(M{=}5)$ cells at $t \in \{2, 3\}$ have gaps
$0.010$ and $0.016$ respectively; mean absolute gap $0.004$).

\item[H2, decode-coincidence invariance under $K$.]
The decode-coincidence attacker's accuracy is invariant under the
canonical-vs-randomized encoding choice, as predicted: the maximum
absolute gap between canonical and random decode rates across all
$36$ $(\text{codec}, M, t)$ cells is $0.0114$, well within the
sampling-noise scale at $n = 2{,}000$ evaluation samples per cell.

\item[H3, Dense saturation boundary.]
For the Dense partition ($M = 3$, $|Y| = 8$, $K(y) = 1$ forced by
codespace saturation), canonical and randomized encoding produce
identical pattern-coincidence rates ($1.0$ at all $t$).  The
randomized-encoding mechanism is inert when the codespace cannot
expand, consistent with the saturation precondition in
Proposition~\ref{prop:rand-encoding}.
\end{description}

The H1 decay rate has a clean operational reading: each doubling of
$|A(y)|$ at fixed $t$ halves the pattern-coincidence rate per
multiplicative factor of $|A(y)|^{t-1}$ in the closed form.  For
deployments at $t = 4$, moving from $M = 4$ (Huffman codespace
class $\approx 4$ per heavy value) to $M = 7$ (class $\approx 32$)
reduces pattern-coincidence accuracy by $230\times$ ($0.4156 \to
0.0018$); the defense is essentially complete at $M = 8$ where
pattern-coincidence accuracy approaches $0$.  Combined with H2,
which confirms randomized encoding does not weaken the
public-codec story, the deployment recommendation is unambiguous:
when shared-$f$ multi-instance composition is unavoidable and the
codec is private, randomized encoding with the largest practical
$|A(y)|$ minimizes pattern-coincidence leakage.
...
three key-universe configurations (Table~\ref{tab:scope-keys})
confirms the scope claim of \S\ref{subsec:threat-scope}:
...
TV-optimal corner separate from the L-optimal Huffman corner; spending
modest $L$ overhead buys only a small ($\approx 2\times$) TV reduction,
while the homophonic multiplicity lever buys an order-of-magnitude
($42\times$) reduction at a storage cost on heavy-tailed value
distributions;
...
(iv) the coincidence-oracle accuracy is maximized by uniform
allocation and minimized by concentrated Huffman-style allocation
(Corollary~\ref{cor:t-geometry}), so the Shannon-optimal partition is
preferred at every $t$ with no codec-choice regime change, and
randomized encoding (Proposition~\ref{prop:rand-encoding}) provides a
free single-instance defense against pattern-coincidence under a
