...
\begin{abstract}
Privacy in encrypted computation need not come from access-pattern hiding
(ORAM), exact algebraic homomorphism (FHE), or simulation-based security
(garbled circuits).  We show that a \emph{cipher map}---a total function on
bit strings where the concepts of domain, correctness, and function identity
exist only behind a one-way trapdoor---provides a distinct, quantifiable
privacy model with predictable composition and information-theoretically
optimal space.  We characterize cipher maps by four independent, measurable
properties (totality, representation uniformity, correctness, composability),
prove the composition theorem governing error accumulation, and present three
constructions---HashSet, entropy map, and trapdoor Boolean algebra---spanning
batch and online strategies.  The Singular Hash Map achieves the
information-theoretic lower bound of $-\log_2 \varepsilon + H(Y)$ bits per
element.
\end{abstract}
...
% IMPROVED: Connect to Bernoulli model error framework
...
respectively, but share the same basic model.  The HashSet construction
(\S\ref{subsec:hashset}) subsumes Bloom filters as a special case: a single
hash function ($k=1$), exact correctness ($\eta=0$), and the same
information-theoretic space bound of $-\log_2\varepsilon$ bits per element.
The difference is that the HashSet is a cipher map---a total function on bit
strings with a trapdoor---while Bloom filters expose their output directly.
...
The Singular Hash Map (\S\ref{sec:singular-hash}) is a perfect hash function
with a different optimization target: it maps domain elements to
...
equality).  Cipher maps differ fundamentally: rather than preserving
structure at the cost of leakage, cipher maps hide structure behind a total
function.  The untrusted machine cannot distinguish domain elements from
noise, and the output distribution is controlled by the representation
uniformity parameter $\delta$.
...
membership through totality and noise closure.
...
The composition of two cipher maps is again a cipher map.  Specifically: let
$(\fhat, \enc_f, \dec_f, s_f)$ be a cipher map for $f : X \to Y$ and
$(\ghat, \enc_g, \dec_g, s_g)$ a cipher map for $g : Y \to Z$.  Then
$\ghat \circ \fhat : \{0,1\}^n \to \{0,1\}^n$, defined by
$(\ghat \circ \fhat)(c) = \ghat(\fhat(c))$, with encoding $\enc_f$ and
decoding $\dec_g$, is a cipher map for $g \circ f : X \to Z$.
\end{definition}
...
cipher maps without needing to decode intermediate results.
...
\begin{proof}
See \S\ref{sec:composition} (Theorem~\ref{thm:composition}).
\end{proof}
...
% IMPROVED: Boolean asymmetry remark — structural constraint on complement
\begin{remark}[Boolean asymmetry]
\label{rem:boolean-asymmetry}
The Boolean algebra homomorphism $F$ preserves union and intersection at the
bit level but does not preserve complement: the complement of a finite set in
the infinite free semigroup $X^*$ covers all hash values (by pigeonhole,
$F(A^\complement) = 1^n$ for any non-trivial $A$).  This asymmetry is
structural, not an artifact of the construction, and constrains all cipher map
systems that compose Boolean operations over hash-based set representations.
\end{remark}
...
\item \textbf{Prefix-free code.}
    ...
    This is the entropy map construction.
...
\begin{algorithmic}[1]
\Function{BuildCipherMap}{$M$, $A$, $h$}
    \Comment{$A$ is an acceptance predicate}
    \State $\ell \gets 0$
    \While{true}
...
$(x, y) \in M$, the hash $h(\ell) \oplus h(x)$ falls in the acceptance
set $A(y)$.  Query evaluation computes $\fhat(c) = h(\ell) \oplus h(c)$
and decodes via the acceptance predicate.

\begin{remark}[Relaxed correctness, $\eta > 0$]
Algorithm~\ref{alg:singular-hash} implements the $\eta = 0$ case.
For $\eta > 0$, accept any seed for which at most
$\lfloor \eta \cdot m \rfloor$ elements fail.  This dramatically reduces
construction time (see \S\ref{subsec:tractability}).
\end{remark}
...
For each stored element $x_i$, the hash $h(\ell) \oplus h(x_i)$ must fall
in the acceptance set $A(y_i)$ (Definition~\ref{def:acceptance}).  Under the
random oracle model, this occurs with probability $\alpha(y_i)$.  Shannon-optimal
allocation sets $\alpha(y_i) \propto p_{y_i}$, giving per-element
information cost $-\log_2 \alpha(y_i) \approx -\log_2 p_{y_i}$ and expected
cost per element $\sum_y p_y (-\log_2 \alpha(y)) = \mu = H(Y)$.
...
falling in \emph{any} acceptance set is
$\varepsilon = \sum_y \alpha(y)$, the noise-decode probability.  Achieving
target rate $\varepsilon$ requires $-\log_2 \varepsilon$ bits of hash
beyond the value encoding.
...
For $m$ stored elements with acceptance predicate $A$, the probability of
successful construction for a given seed $\ell$ is:
\begin{equation}
p = \prod_{i=1}^{m} \alpha(y_i),
\end{equation}
where $\alpha(y_i) = |A(y_i)| / 2^n$ is the acceptance probability for the
$i$-th element's target value.  The expected number of trials is $1/p$.
\end{proposition}
...
element $x_i$, the hash falls in $A(y_i)$ with probability $\alpha(y_i)$.
Independence across elements gives the product formula.
The number of trials is geometric with success probability $p$.
\end{proof}

For uniform acceptance probability $\alpha(y) = \varepsilon / |Y|$, this
gives $p = (\varepsilon / |Y|)^m$ and expected trials
$(|Y| / \varepsilon)^m$---intractable for even modest $m$.
...
\begin{proposition}[Bucketed construction time]
\label{prop:bucketed}
With $k$ buckets of expected size $m/k$, the expected total construction
time is
\begin{equation}
...
\eta_{g \circ f} \leq \eta_f + \eta_g - \eta_f \eta_g
  = 1 - (1 - \eta_f)(1 - \eta_g).
\end{equation}
Equality holds when $\fhat$ and $\ghat$ use independent seeds and
$\ghat$'s correctness on $\fhat(c)$ is independent of whether $\fhat$
was correct (the re-randomization condition).
\end{theorem}

\begin{proof}
The composition is correct when \emph{both} maps produce correct results.

\emph{Upper bound.}  By the union bound,
$\Pr[\text{incorrect}] \leq \eta_f + \eta_g$.
Since $\eta_f \eta_g \geq 0$, the tighter bound
$\eta_f + \eta_g - \eta_f \eta_g$ also holds.

\emph{Equality under re-randomization.}
If the re-randomization condition holds,
$\Pr[\text{both correct}] = (1-\eta_f)(1-\eta_g)$,
giving $\eta_{g \circ f} = 1 - (1-\eta_f)(1-\eta_g)$.

In practice, $\fhat$ produces a \emph{specific} encoding of $f(x)$, not a
uniformly random one.  If $\ghat$'s correctness depends on which encoding
it receives, the errors may be positively correlated, making the inequality
strict.  For specific circuits, the case-by-case interval arithmetic from
...
% IMPROVED: Type hierarchy and scope of the present formalism
...
untrusted provider---is a direct application of the cipher map abstraction.
The domain-specific vocabulary maps to the general framework as follows.

\begin{description}
\item[Search agent] $=$ trusted machine $T$.  Holds the trapdoor $s$,
    encodes queries, decodes results.
\item[Encrypted search provider] $=$ untrusted machine $U$.  Holds cipher
    maps and cipher values; evaluates $\fhat(c)$ for any
    $c \in \{0,1\}^n$ without decoding.
\item[Information need] $=$ latent function $f : X \to Y$.  For keyword
    search, $f = \mathbf{1}_A$ (set indicator); for ranked retrieval, $f$
    is a scoring function.
\item[Hidden query] $=$ cipher value $\enc(x, k)$.  The provider evaluates
    the cipher map on this value without learning $x$.
...
\item \textbf{Totality}: The provider can evaluate the index on any bit
    string; filler queries produce output indistinguishable from real queries.
\item \textbf{Representation uniformity}: Hidden queries are $\delta$-close
    to uniform; frequency analysis of query traffic is bounded by $\delta$.
\item \textbf{Correctness}: The search agent recovers the correct answer
    with probability $\geq 1 - \eta$; nonzero $\eta$ provides plausible
    deniability.
\item \textbf{Composability}: Boolean combinations of queries (AND, OR, NOT)
    compose as cipher map chains with predictable error accumulation.
\end{enumerate}

The correctness parameter $\eta$ has a precise deniability interpretation
for Boolean-valued cipher maps.

\begin{proposition}[Bayesian deniability]
\label{prop:deniability}
Let $\fhat$ be a cipher map for $f : X \to \{0,1\}$ with correctness
$\eta$, and let $\pi = \Pr[f(x) = 1]$ be the prior.  If the adversary
observes the decoded output $y = 1$, then
\begin{equation}
\Pr[f(x) = 1 \mid y = 1]
    = \frac{\pi(1-\eta)}{\pi(1-\eta) + (1-\pi)\eta}.
\end{equation}
For $\eta = 0$, the posterior equals $1$ (no deniability).  For
$\eta = 1/2$, the posterior equals $\pi$ (observation is useless).
\end{proposition}

\begin{proof}
By Bayes' rule: $\Pr[y=1 \mid f(x)=1] = 1-\eta$ (correct positive) and
$\Pr[y=1 \mid f(x)=0] = \eta$ (false positive).  Applying Bayes' theorem
with prior $\pi$ gives the stated formula.
\end{proof}
...
% IMPROVED: Sum-type confidentiality trade-off
