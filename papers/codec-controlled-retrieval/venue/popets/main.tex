...
invariance under within-class re-encoding, under a saturation precondition) for the abstract
...
$\ell$ and each $q'(v) = h_v\,2^{-\rank\proj_\ell(\W)} = q(v)$. That a full projection cannot be
lost to a short-value perturbation is the one step we establish computationally rather than in
closed form: it is verified exhaustively on the full-support store and over $12{,}000$ redraws
across five codecs, with a general closed-form argument for arbitrary complete codecs left open.
Saturation is sufficient, not necessary (storing only $A$ keeps every draw in $\class(A)$,
invariant yet unsaturated).
