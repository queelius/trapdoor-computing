...
% PoPETs submission format. acmart sigconf is the PoPETs 2-column base; for the final
% upload add the official \usepackage{popets} from the PETS author kit (it adjusts the
% masthead/metadata only, not the body geometry). acmart already loads amsmath, amsthm,
% booktabs, hyperref, and natbib.
\settopmatter{printacmref=false, printfolios=true}
\setcopyright{none}
\renewcommand\footnotetextcopyrightpermission[1]{}
\citestyle{acmnumeric}
\usepackage{mathtools}
\let\Bbbk\relax % amssymb and acmart's math font both define \Bbbk
\usepackage{amssymb}
\usepackage{cleveref}

% acmart predefines theorem, lemma, corollary, proposition, definition, and example,
% but not remark, so we add it here.
...
\title{Codec-Controlled Retrieval: Structural Frequency-Hiding from the Non-Member Channel of XOR Retrieval}
...
for \emph{every} key, with no membership gate: the stored value for a member, and
what the literature calls an arbitrary value, junk, a don't-care, for everyone else.
We show that junk is a designable object. For homogeneous ribbon (XOR) retrieval the
value returned for a non-member key is uniform on the $\GF$ span $\W$ of the stored
codewords, so a public value codec, not the private data, fixes its distribution, and
that distribution is provably independent of how often each value was stored. For a
balanced codec, full codec control holds by a sharp threshold: it succeeds if and only
if the stored codewords are transversal to the codec's class partition, equivalently
$\rank \proj|_{\W} = \log_2 K$, a step in an integer rank with no graded middle (below
it whole codec classes receive exactly zero probability). A second code invariant
governs robustness: the matroid cogirth $d^\ast$ of the stored column system is the
exact adversarial erasure budget before control collapses, at least
$\tfrac{K}{2}\,m_{\min}$ under full support, so rank governs control while cogirth
governs its robustness. We cash this out as a structural frequency-hiding property. In
a distinguishing game (FreqDist) an adversary with non-member oracle access has
advantage exactly zero in the idealized model, for any number of adaptive queries,
since the two oracles are then the identical law. In the real construction the two
non-member laws lie within total variation $\delta(p_0)+\delta(p_1)$, so a single
observation distinguishes them by at most that distance, and more queries only sharpen
an estimate of two fixed laws whose separation does not grow with the frequency gap; here $\delta$
is a small, scale-independent, redundancy-governed deviation we measure to straddle the
query-sampling floor ($0.00334$ at $10^5$ queries) up to $10^7$ keys, at zero per-query
cost, where online encrypted-database defenses pay per query. A second,
multi-instance coincidence oracle pulls the codec the opposite way, defeated by
\emph{concentrated} rather than flat codespace shares, so there is no single optimal
codec, only a public tunable frontier. The construction is a header-only C++23
implementation, and every theorem has a passing empirical check at scale.
\end{abstract}

\keywords{retrieval data structures, frequency hiding, XOR retrieval, GF(2) linear
algebra, leakage suppression, structured encryption}
...
off-set output distribution uncharacterized, and the closest mechanical cousin, learned static
functions~\cite{hermann2025learned} that build per-key prefix codes over a value distribution,
spends those codes compressing \emph{member} values rather than shaping the \emph{non-member}
output.

We fill exactly that silence: the non-member output follows a law fixed by the public
codec on the stored $\GF$ span, not a don't-care.
...
\begin{itemize}
  ...
  allocation can shape a non-member output distribution is \emph{known}. It is a
  prior result for a random-oracle singular-hash-map
  ...
  \item \textbf{(D, refinements and robustness.)} Three refinements, developed together in
  \cref{sec:refinements}, complete the picture. For a skewed (variable-length) codec,
  control is \emph{graded} rather than sharp: per-class mass is a non-increasing ladder in
  code length with a provably unreachable uniform target (\cref{thm:graded}), and the full
  realizable set is characterized constructively (\cref{thm:realizable}). A second code
  invariant governs robustness: the matroid cogirth $d^\ast$ of the stored column system is
  the exact, tight adversarial erasure budget (control survives every deletion of fewer than
  $d^\ast$ stored keys and collapses at exactly $d^\ast$), at least $\tfrac{K}{2}\,m_{\min}$
  under full support (\cref{thm:cogirth}), so \emph{rank} governs control while
  \emph{cogirth} governs its robustness; for a skewed codec this becomes a per-rung family
  of cogirths (\cref{thm:cogirth-graded}). And storing a random within-class representative
  leaves the non-member law exactly invariant while diverging the per-build solution,
  snapshot confidentiality at no per-query cost (\cref{thm:randenc}).
\end{itemize}
On the security side, we formalize a distinguishing game (FreqDist) in which the
idealized adversary advantage is \emph{exactly zero} (\cref{thm:freqdist-ideal}), for any
number of adaptive queries, because the two oracles are then the identical law. In the
real construction the two non-member laws are within total variation
$\delta(p_0)+\delta(p_1)$ of each other, so a single observed output distinguishes them
with advantage at most that distance (\cref{thm:freqdist-real}), and additional queries
only sharpen an adversary's estimate of two fixed laws whose
separation stays $\delta$-bounded; here $\delta$ is the small, scale-independent deviation
of the real construction from the idealization, measured to straddle the query-sampling
floor up to ten million keys.
We implement the construction
...
\[
  \begin{aligned}
  &\text{M1:}\quad \ay^{\mathsf T}\bz \;\sim\; \Uniform(\W),
  \quad\text{i.e.}\\
...
of ribbon retrieval in the abstract. A computational gate (tag
\texttt{[span][raw]}) confirms the inclusion falsifiably: it drives the ribbon directly
with the canonical right-hand sides and asserts the \emph{undecoded} $M$-bit output of
$30{,}000$ distinct non-member queries lies in $\W$, where only $4$ of the $16$ possible
raw patterns do ($2$ of $16$ in a companion sub-threshold instance). A
decode-then-re-encode check cannot serve here: for a Kraft-tight codec whose canonical
patterns all lie in $\W$, the re-encoded pattern lands in $\W$ for \emph{every}
conceivable raw output, so such a check verifies only the class-level law.
\end{remark}
...
deviation from $\Uniform(\W)$ characterized in \cref{sec:t5}. The scope of that rider,
stated carefully: the idealized law contains no frequency term at all (the theorem
above), and the deviation contains no term growing with the frequency \emph{gap}
between two profiles (E2: the cross-profile distance sits at the same-profile rebuild
floor); but the deviation's \emph{magnitude} does depend on per-class storage
redundancy, a function of the $m_v$ (thin support elevates $\delta$, the effect
\cref{sec:t5} characterizes and the cogirth theorems explain). An observer of an
elevated deviation can learn that some class is thinly stored; what it cannot learn is
the direction or shape of the frequency vector.
...
storage frequencies $99/0.5/0.5$ and $50/25/25$ (percent), a raw frequency
...
This completes the balanced codec-control threshold. The skewed (variable-length)
codec, the robustness of control under adversarial erasure, and a randomized-encoding
variant all refine this picture, but none is needed for the security payoff; we develop
them in \cref{sec:refinements}, after the idealized-to-real gap (\cref{sec:t5}) and the
FreqDist result (\cref{sec:security}).
...
\cref{thm:cogirth}, developed in \cref{sec:refinements}, proves that the rank making control possible persists under
...
\subsection{The two real laws are close, so the per-observation advantage is small}
...
The two real non-member laws are close in total variation, independently of the frequency gap and
regardless of how many samples are drawn:
\[
  \TV\big(\lawreal(p_0), \lawreal(p_1)\big) \;\le\; \delta(p_0) + \delta(p_1).
\]
Consequently, in the convention $\Adv = |2\Pr[b'=b]-1|$, the single-observation
distinguishing advantage (the advantage of the optimal test on one observed non-member
output) is
\[
...
two \emph{fixed} laws; their separation, capped by $\delta(p_0)+\delta(p_1)$, contains no
...
\[
  ...
  &\le \TV(\lawreal(p_0), \lawM)\\
  &\quad{} + \TV(\lawM, \lawreal(p_1))\\
  &= \delta(p_0) + \delta(p_1).
  \end{aligned}
\]
...
A many-query adversary draws outputs from one fixed instance and can estimate that
instance's realized law to arbitrary precision. We are precise about what that buys:
closeness of the two laws does \emph{not} by itself cap the many-query advantage, since two
realized-law \emph{distributions} (over builds) can each lie within $\delta$ of the ideal yet
be nearly disjoint from each other inside that ball, in which case the estimated law would
identify the profile. What forecloses that here is structural plus measured: by
\cref{thm:freqindep} the law contains no multiplicity term at all, so the per-build deviation
is redundancy-and-seed noise rather than a profile signature, and a same-profile rebuild under
a fresh ribbon seed occupies the same $\delta$-ball (E2 in \cref{sec:experiments} measures the
cross-profile distance to coincide with this per-build rebuild floor). So the many-query
ceiling, the separation between the two realized-law distributions, is \emph{measured} at the
per-build floor rather than proven in closed form; the proven content is the displayed
single-observation bound, together with the structural fact that \cref{thm:freqindep} leaves
no frequency-gap signal in the law for sampling to surface.
\end{proof}
...
\section{Refinements: the skewed regime, robustness, and randomized encoding}
\label{sec:refinements}

The balanced threshold and its security payoff are now complete: the non-member law
(\cref{thm:idealized}), its frequency independence (\cref{thm:freqindep}), the sharp
control threshold (\cref{thm:threshold}), the idealized-to-real gap (\cref{sec:t5}),
and the FreqDist result (\cref{sec:security}). Three refinements extend this picture
without changing it; we summarize them here and develop them in full, with proofs, in
\cref{app:refinements}. First, a skewed (variable-length) codec makes control
\emph{graded} rather than sharp: per-class mass is a non-increasing ladder in code
length, a uniform target is unreachable, and the realizable set is characterized
completely (\cref{thm:graded,thm:realizable}). Second, the robustness of control is a
second code invariant: the matroid cogirth $d^\ast$ of the stored column system is the
exact adversarial erasure budget, at least $\tfrac{K}{2}\,m_{\min}$ under full support,
so rank governs control while cogirth governs its robustness, with a per-rung family for
skewed codecs (\cref{thm:cogirth,thm:cogirth-graded}). Third, storing a uniform
within-class representative leaves the non-member law exactly invariant (unconditionally
for balanced codecs, under a saturation condition for skewed) while diverging the
per-build solution: snapshot confidentiality at no per-query cost (\cref{thm:randenc}).
...
an open-source research codebase; because its development branch moves, the artifact pins a
...
$\delta(p_0)+\delta(p_1)$ and that caps the per-observation advantage; its sitting at the
same-profile rebuild floor is also the measured evidence that many-query estimation
surfaces no profile signal (the separation between the realized-law distributions is the
rebuild noise itself).
...
\begin{table}[t]
  \centering
  \caption{E3: mean TV-to-codespace versus stored-key count $N$. The deviation is consistent
  with being flat in $N$ from $10^4$ to $10^7$ (intervals overlap, though wide and
  low-replication at $N \ge 10^6$), and straddles the analytic query-sampling floor $0.00334$
  (inside every interval, below it at $N = 10^7$). The ribbon's $\epsilon$ auto-scaling at
  $10^7$ keys does not degrade control.}
  \label{tab:e3}
  \begin{tabular}{lccc}
    \toprule
...
scale) and consistent with being flat in $N$: the intervals overlap, though the two
largest-$N$ rows rest on only $5$ and $3$ build seeds, so those intervals are wide and the
normal approximation is loose. The analytic sampling floor for these
...
query budget, consistent with \cref{sec:t5}; the scale-independence reading rests on this
floor comparison rather than on the inter-$N$ interval overlap. Replication is cost-bounded
(a $10^7$-key build takes about $30$ s), so we use $10$ seeds up to $10^5$ keys and fewer
above. (E1 and E2 use $5 \times 10^4$ non-member queries per build, for a higher floor of
about $0.0047$; the $0.00334$ floor quoted here is E3's, at $10^5$ queries.)
...
\begin{table}[t]
  ...
  every cell (within a $95\%$ interval, or one binomial standard error near saturation),
  validating $\mathrm{acc}(t) = 1 - S(t)/2$.}
  \label{tab:e4}
  \resizebox{\columnwidth}{!}{%
  \begin{tabular}{lccc}
    \toprule
    ...
    intermediate\_zipf $(\sim 0.43)$ & $0.98240$ & $0.98255$ & yes \\
...
We position the contribution honestly, including against closely related prior work, so
...
prior singular-hash-map work~\cite{towell_bernoulli_maps} derives exactly this formula
...
Four adjacencies deserve an explicit boundary. \textbf{(1) A prior
singular-hash-map construction}~\cite{towell_bernoulli_maps} contains the identical formula but for a
random-oracle construction; the transfer to a linear structure, and the discovery that the
transfer is gated by a provable span threshold, is the new content. \textbf{(2) The companion
cipher-maps framework}~\cite{towell2026ciphermaps} shares the frequency-hiding goal, but its
...
property for it. That companion also states a randomized-encoding invariance (decoded-law
invariance under within-class re-encoding, under a support precondition) for the abstract
cipher-map model; what \cref{thm:randenc} adds is specific to the linear backend, namely the
per-build divergence of the white-box solution $\bz$, which has no analogue where the output is
a fresh hash, and the exact skewed-codec failure boundary that the abstract statement leaves as
an unexamined precondition. \textbf{(3) Honey-Encryption DTE}~\cite{juels2014honey,cheng2019pmte}
...
access or encoding distribution at query or insert time (fake accesses, padding). The line is
active in both directions, with tunable system-wide successors~\cite{zheng2024swat} and stronger
attacks against them: IHOP~\cite{oya2022ihop} recovers queries and degrades PANCAKE-style
smoothing once the query stream is correlated. Those attacks target the online access stream;
our property is static and structural, a different layer: the non-member law of a built linear
structure is frequency-independent by construction (\cref{thm:freqindep}), with no online
smoothing, no fake traffic, no padding, so a correlated query stream has no purchase on the
non-member value channel.
...
The broader backdrop is the leakage lineage from searchable and structured encryption that motivates
...
distinguishing adversary with non-member oracle access has advantage exactly zero in the
idealized model for any number of adaptive queries (\cref{thm:freqdist-ideal}); in the real
construction the two non-member laws are within total variation $\delta(p_0)+\delta(p_1)$, a
fixed separation, independent of the frequency gap, that no number of queries can widen, so a single
observation distinguishes them by at most that distance (\cref{thm:freqdist-real}), a property
of the static bytes with zero per-query cost, where PANCAKE and FSE pay bandwidth or re-encoding
online.
...
general and holds under full support, with the intermediate saturation condition refuted). The \emph{magnitude} of the deviation
...
encoding invariance beyond the full-support sufficient condition. For the
...
broadly, this work is a construction-and-experiments companion to the cipher-maps
framework~\cite{towell2026ciphermaps}: it supplies the $\GF$-linear retrieval backend that
...
\section{Refinements: full development}
\label{app:refinements}

This appendix gives the full statements and proofs of the three refinements summarized
in \cref{sec:refinements}: the graded skewed-codec regime and its realizable set
(\cref{thm:graded}, \cref{thm:realizable}), the cogirth robustness budget
(\cref{thm:cogirth}, \cref{thm:cogirth-graded}), and the randomized-encoding invariance
(\cref{thm:randenc}).
...
\begin{equation}
  \begin{aligned}
  q(v) &\;=\; \frac{|\class(v) \inter \W|}{|\W|}
        \;=\; h_v \cdot 2^{\,\dim(\W \inter C_{\ell_v}) - \dim \W},\\
       &\text{where } h_v = [\, \enc(v) \in \W + C_{\ell_v} \,].
  \end{aligned}
  \label{eq:graded}
\end{equation}
...
\begin{enumerate}
  \item \emph{(Full-span control reproduces the Kraft law.)} When $\W = \GF^M$, every class is hit and
...
For (2), the shortest class $A$ (length $1$) is always hit (its codeword is the
all-zero pattern, so $\enc(A) = 0 \in \W + C_1$ for every $\W$, condition (c) of
\cref{thm:realizable}; dimension alone would not give this) and $q(A) = 2^{d_1 - \dim \W}$
...
degenerate) or $\dim \W - 1$ ($q(A) = 1/2$); neither is $1/4$. Since $q(A) \in \{1/2, 1\}$
while uniform places $1/4$ at $A$, the excess mass at $A$ is at least $1/4$; as total variation
equals the total positive excess, $\TV \ge 1/4$ to uniform, over all
subspaces $\W$. The full enumeration (every $\GF$ subspace for $M = 4$ and $M = 6$) is in
\cref{app:proofs}.
\end{proof}

\begin{example}[A graded mass vector]
\label{ex:graded}
...
check asserts \eqref{eq:graded}, the ladder, the full-span Kraft law, and uniform-unreachability
...
and the ladder; these necessary conditions are \emph{sufficient} for some codecs (for skew
...
\emph{multiset}: without multiplicity the budget cannot see per-class redundancy (a class
stored a thousand times and one stored once contribute the same single column), so the count
would be a function of the support alone. Define $d^\ast$ to be the matroid \emph{cogirth} of $G$, that is, the
...
\begin{enumerate}
  ...
  \item \emph{(Corruption budget.)} $d^\ast$ is also the exact \emph{substitution} budget:
  control survives every corruption of fewer than $d^\ast$ stored keys' values (for any
  replacement values), and $d^\ast$ well-chosen substitutions break it.
...
deleting exactly the $d^\ast$ columns outside $H$ leaves all survivors in $H$, of rank
exactly $k-1$ (their span cannot be smaller: a rank-$\le k{-}2$ survivor span, extended by one
column, would still fit in a hyperplane capturing strictly more columns than $H$,
contradicting maximality in \eqref{eq:cogirth}). The minimum breaking deletion is therefore
exactly $d^\ast$, the minimum distance of the code generated by the rows of $G$ (the dual
distance of the code $G$ checks). The value in the broken regime is pinned by \cref{thm:threshold}: at surviving rank
...
\emph{(2)} Persistence: with fewer than $d^\ast$ corruptions the untouched columns survive a
deletion of size $< d^\ast$, hence still span $\Quot$ by (1), and the corrupted system contains
them, so rank stays $k$. Tightness: substitute the $d^\ast$ keys outside a maximizing
hyperplane with values whose patterns lie inside it (possible, as every hyperplane holds $K/2$
patterns), dropping the rank to $k-1$. (This replaces an earlier ``$\lfloor(d^\ast-1)/2\rfloor$
errors correctable'' claim, which mis-transported the decoding radius: here every column
multiset is a legal build, so no code constraint anchors nearest-neighbor decoding.)
...
\[
  \begin{aligned}
    &\textsc{rank governs control} \quad\text{(\cref{thm:threshold})},\\
    &\textsc{cogirth governs robustness} \quad\text{(\cref{thm:cogirth})}.
  \end{aligned}
\]
...
\begin{enumerate}
  \item \emph{(Per-rung budget.)} For a \emph{hit} rung with $\rank \proj_v(\W) \ge 1$ let
  $d^\ast_v$ be the matroid cogirth of the $\proj_v$-projected column multiset
  (\eqref{eq:cogirth} read on the image $\proj_v(\W)$). Then $d^\ast_v$ is the exact minimum
  number of stored-key erasures that changes $q(v)$: fewer leave it invariant; at $d^\ast_v$ it
  doubles (if $v$ stays hit) or collapses to $0$ (if the rank drop also drops $h_v$). A
  \emph{pinned} hit rung ($q(v) = 1$, $\rank \proj_v(\W) = 0$, the cascade's terminal state)
  has $d^\ast_v = \infty$ and never changes.
  ...
  the \emph{first} threshold of this single-rung case; the cascade itself does not collapse to
  one step but continues through ranks $k{-}1,\dots,0$ (e.g.\ $\{2,2,2,2\}$ at $m=3$ changes at
  $6$ and again at $9$ erasures).
\end{enumerate}
\end{theorem}
...
columns have a set top bit, so all $9$ must be erased to drive $\proj_A(\W)$ to zero and flip
$A$); the three thin length-$3$
...
What does it buy to store a \emph{random} within-class representative of each value, in place
of the canonical one, and at what cost? \Cref{rem:genericity} fixed the builder to inject no
pattern outside $\W$: it stores the \emph{canonical} codeword $\enc(v)$ of each value, the
unique class representative whose within-class bits are zero. The codec already exposes the
...
\begin{theorem}[Randomized encoding: balanced invariance, skewed full-support boundary, divergence]
\label{thm:randenc}
...
\begin{enumerate}
  ...
  \item \emph{(Skewed, false in general; full support sufficient.)} For a skewed codec the law
  ...
  occur). For a \emph{complete} (Kraft-tight) codec, invariance holds for \emph{every} draw
  whenever the store has \emph{full support} (every value stored at least once); then every draw
  realizes the designed Kraft law $q(v) = 2^{-\ell_v}$. Full support is sufficient, not necessary.
  The weaker \emph{saturation} condition ($\proj_\ell(\W) = \GF^\ell$ for all $\ell$) is
  \emph{not} sufficient: on $\{2,2,2,3,3\}$ the saturated store $\{B,C,E\}$ has $16$ of its $32$
  draws change the law.
...
$\ell$. \emph{Saturation does not suffice}: a full $\proj_\ell(\W)$ can owe its fullness to
short generators' images, which the redraw moves (the $\{B,C,E\}$ counterexample above).
\emph{Full support does}, by sibling contraction on the complete code tree: the deepest
codewords pair as siblings, both stored under full support, with $\proj_{\ell_{\max}}$ images
fixed (a length-$\ell_{\max}$ draw moves only bits $\proj_{\ell_{\max}}$ discards) and differing
by the lowest image coordinate $e$; so $e \in \proj_{\ell_{\max}}(\W')$ for every draw.
Quotient by $e$: each deepest pair contracts to its (fixed-image) parent, a leaf of a complete
depth-$(\ell_{\max}-1)$ code that still has full support, and induction gives
$\proj_{\ell_{\max}-1} = \GF^{\ell_{\max}-1}$ on the quotient; with $e$ this spans
$\GF^{\ell_{\max}}$, so every $\proj_\ell(\W')$ is full and $q'(v) = 2^{-\ell_v} = q(v)$. (Base
case $\ell_{\max}=1$: codewords $0,1$.) Sufficiency, not necessity: storing only $A$ keeps every
draw in $\class(A)$, invariant yet not full support.
...
(balanced, or skewed under full support) case; a skewed codec is a flag of cosets of different
subspaces, and randomizing within a short value's coset is visible to a longer value's projection.
Two consequences follow. (C1) When invariance applies, the entire non-member value-frequency
...
This holds unconditionally for a balanced codec and, for a skewed codec, under full support;
under \emph{partial} support a skewed codec is not off-set invisible (even when saturated), so
balance or full support is a precondition for using within-class randomization as a
...
$a^{\mathsf T}\bz$. This is an entropy statement, not a snapshot-indistinguishability bound:
the snapshot still determines $\W'$ and every $\proj_\ell(\W')$, so white-box leakage beyond the
off-set law is not excluded here. The \texttt{[randomized]} check gates the balanced/full-support
claims at fixed seeds: exact mass invariance over $1300$ within-class re-encodings across four
codecs (asserted by integer cross-multiplication, while $|\W|$ grows from $8$ to $16$ on the skew
...
non-member law is unchanged; every store there has full support. A separate
\texttt{[randomized][boundary]} check exhibits the skewed counterexample (store
$\{A,A,B\}$ on skew $\{1,2,3,3\}$: four distinct laws over $256$ draws), the full-support theorem
($128$ draws of $\{A,B,C,D\}$, $256$ of the $\{2,2,2,3,3\}$ full store, all reproduce the law), and
the \emph{saturation refutation} (the saturated store $\{B,C,E\}$: $16$ of $32$ draws break).
...
{\footnotesize
\begin{verbatim}
solution.assign(num_rows, 0);         // (i)
for (col = num_rows; col-- > 0; ) {
  if (pivot_coeffs[col]==0) continue; // (ii)
  val = pivot_value[col];             // (iii)
  ... val ^= solution[ref] ...        // (iv)
  solution[col] = val;                // (v)
}
\end{verbatim}
...
$\ge$ share and the ladder yet no subspace realizes it, so in general the constructive
...
In keeping with the venue's disclosure policy, we report our use of large language model
(LLM) based tools in preparing this manuscript. \emph{Drafting and editing:} an agentic
multi-tool writing-and-review system was used to reshape pre-existing, human-developed
technical results into venue prose, to copy-edit, and to run editorial review passes;
the authors reviewed and approved all resulting text. \emph{Verification:} every theorem,
proof, bound, and numerical value was checked by the authors against the computational
gates and committed result files of the accompanying artifact, and no theorem statement
or experimental number originated from an LLM. \emph{Literature and bibliography:}
LLM-assisted web search was used to locate related work and to draft some BibTeX entries,
each of which was then verified against primary sources; one LLM-introduced bibliographic
error (an incorrect author list) was caught during this verification and corrected. The
authors take full responsibility for the entire manuscript, including all claims, proofs,
measurements, and citations; no AI system is an author.
