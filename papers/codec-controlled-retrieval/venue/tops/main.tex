...
% ACM TOPS submission format (acmart acmsmall, single-column journal). For the real
% submission, confirm the TOPS review model: if double-anonymous, add `anonymous` to the
% documentclass and apply the anonymization deltas (see NOTES.md). acmart already loads
% amsmath, amsthm, booktabs, hyperref, and natbib.
\acmJournal{TOPS}
...
and the FreqDist result (\cref{sec:security}). This section develops three refinements
the spine deferred. The first treats a skewed, variable-length codec, where control is
graded rather than sharp. The second asks how robust control is to adversarial erasure
of stored rows, and answers with a second code invariant, the cogirth. The third asks
what a randomized within-class encoding buys, and finds snapshot confidentiality at no
per-query cost. Each refines the threshold picture without changing it.
...
In keeping with ACM's policy on generative AI, we report our use of large language model
...
technical results into journal prose, to copy-edit, and to run editorial review passes;
the authors reviewed and approved all resulting text. \emph{Verification:} every theorem,
proof, bound, and numerical value was checked by the authors against the computational gates
and committed result files of the accompanying artifact, and no theorem statement or
experimental number originated from an LLM. \emph{Literature and bibliography:} LLM-assisted
web search was used to locate related work and to draft some BibTeX entries, each of which
was then verified against primary sources; one LLM-introduced bibliographic error (an
incorrect author list) was caught during this verification and corrected. The authors take
full responsibility for the entire manuscript, including all claims, proofs, measurements,
and citations; no AI system is an author.
