...
distinguishing game (FreqDist) an adversary with non-member oracle access has
advantage exactly zero in the idealized model and at most $2(\delta_0+\delta_1)$ in
the real construction, where $\delta$ is a small, scale-independent,
...
$1-\tfrac12\sum_v \al(v)^t$) pulls the codec the opposite way, so there is no single
optimal codec, only a public tunable frontier. The construction is a header-only
...
the stored value in space close to the information-theoretic minimum. For a key not
...
correct on the member set. Nobody characterizes what that arbitrary value
\emph{is}. As recently as 2025, the state of the art in static
...
\begin{itemize}
  ...
  probability. This threshold appears nowhere we are aware of, internal or external.
\end{itemize}
On the security side, we formalize a distinguishing game (FreqDist) in which the
idealized adversary advantage is \emph{exactly zero} (\cref{thm:freqdist-ideal}) and
the real advantage is at most $2(\delta(p_0)+\delta(p_1))$
(\cref{thm:freqdist-real}), where $\delta$ is the small, scale-independent deviation
of the real construction from the idealization, measured to be at or below the
query-sampling floor up to ten million keys. We implement the construction
...
The builder solves the banded $\GF$ system so that, for every member $x \in S$,
\begin{equation}
...
\subsection{The mass on each class is $0$ or $1/K'$}
...
distinguish: the two oracles are one and the same law.

\subsection{Real advantage at most twice the summed deviation}
...
(the same object for both, being frequency-independent by \cref{thm:freqindep}).

\begin{theorem}[FreqDist, real]
\label{thm:freqdist-real}
$\Adv(\mathcal{A}) \le 2\big(\delta(p_0) + \delta(p_1)\big)$ for every adversary.
\end{theorem}

\begin{proof}
The standard fact that distinguishing advantage is bounded by total variation gives,
for any adversary observing samples from one of the two laws,
$\Adv(\mathcal{A}) \le 2\,\TV(\lawreal(p_0), \lawreal(p_1))$. (The factor $2$ is the
convention of
$\Adv = |2\Pr[b'=b]-1|$; with $\Adv = |\Pr[b'=b]-\tfrac12|$ the bound reads $\le \TV$, and
the optimal single-sample distinguisher achieves exactly the TV.) Routing through the
common M1 law and using the triangle inequality for TV,
\[
  \TV(\lawreal(p_0), \lawreal(p_1))
  \le \TV(\lawreal(p_0), \lawM) + \TV(\lawM, \lawreal(p_1))
  = \delta(p_0) + \delta(p_1),
\]
so $\Adv(\mathcal{A}) \le 2(\delta(p_0) + \delta(p_1))$.
\end{proof}

Plugging in the \cref{sec:t5} numbers: with both profiles rich,
$\delta(p_0) \approx \delta(p_1) \approx 0.003$, so $\Adv \le 2(0.003 + 0.003) = 0.012$;
with one profile at the threshold edge (the skew\_a regime), $\delta \approx 0.017$ on the
thin side and $\approx 0.003$ on the rich side, so
$\Adv \le 2(0.017 + 0.003) = 0.04$. E2 (\cref{sec:experiments}) confirms not just the
bound but its \emph{shape}: the measured cross-profile advantage sits at the per-build
floor and does not grow with the frequency gap between $p_0$ and $p_1$, exactly as
$2(\delta(p_0)+\delta(p_1))$ predicts and the idealized $\Adv = 0$ demands.
...
\begin{itemize}
  ...
  with non-member oracle access cannot learn the storage-frequency distribution of values
  in $S$. Idealized advantage zero (M1, from \cref{thm:idealized,thm:freqindep}); real
  advantage at most $2(\delta(p_0)+\delta(p_1))$, a small constant governed by storage
  redundancy and independent of the frequency gap (\cref{sec:t5}, E2).
...
at the threshold edge), a \cref{sec:t5} effect, not a frequency-leakage signal. This jointly
confirms the independence (advantage does not grow with the frequency gap) and the
redundancy characterization (advantage set by the per-class store count of the thinner
build), realizing the real bound $\Adv \le 2(\delta(p_0)+\delta(p_1))$ of
...
the member set, and its distribution is never studied. As of 2025 the state of the art in
static retrieval~\cite{hu2025retrieval} leaves it uncharacterized. Characterizing that
distribution, and showing it is the codec's own codespace law gated by the span threshold, is
the new content.
...
transversality of $\W$ to the codec's class partition. This appears nowhere we are aware of,
internal or external: absent from the random-oracle treatments (Tier A, where there is no span
to be transversal to), and absent from the retrieval literature (Tier B, which does not study
the non-member law at all). The subspace-meets-coset lemma (\cref{lem:subspace-coset}) and the
resulting $K' = |\proj(\W)|$ step function are the technical core, and the cliff in E1 is its
measured face.
...
idealized model (\cref{thm:freqdist-ideal}) and at most $2(\delta(p_0)+\delta(p_1))$ in the real
one (\cref{thm:freqdist-real}), a property of the static bytes with zero per-query cost, where
