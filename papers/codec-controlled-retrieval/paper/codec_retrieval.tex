...
governs control while cogirth governs its robustness. We cash this out as a structural
...
\begin{itemize}
  ...
  that \emph{rank} governs control and \emph{cogirth} governs its robustness.
\end{itemize}
...
sufficient), and the skewed analogue of the robustness theorem below.
...
\cref{thm:threshold}'s scope); the skewed/graded extension is a flagged follow-on.
...
skewed (variable-length) case is resolved as the \emph{graded} regime (\cref{thm:graded}), but
two finer skewed questions remain open: a complete characterization of the realizable mass
set (the non-increasing ladder is necessary, not sufficient), and the skewed analogues of the
cogirth budget and of the randomized-encoding invariance (\cref{thm:randenc}, proven for
balanced and only computationally verified for skewed). The \emph{magnitude} of the deviation
...
Three resolved items leave sharper successors. The skewed codec is now \emph{graded}
(\cref{thm:graded}); what remains is a complete characterization of its realizable mass set
(the ladder is necessary, not sufficient) and the skewed analogues of the cogirth budget
(\cref{thm:cogirth}) and of the randomized-encoding invariance (\cref{thm:randenc}), each of
which we prove for the balanced codec and verify computationally in the skewed case. For the
