...
within-class representative leaves the entire non-member law exactly invariant while
diverging the white-box solution per build, snapshot confidentiality at zero per-query
cost. We are honest about the perimeter:
...
We also show that randomizing the within-class representative of each stored value leaves
the entire non-member law exactly invariant while diverging the white-box solution per
build (\cref{thm:randenc}), a snapshot-confidentiality property obtained at zero per-query
cost. We implement the construction
...
So the skewed regime trades the balanced ``flat or nothing'' dichotomy for a controllable
ladder whose extreme point is the designed Kraft distribution itself. What remains open
is finer: a complete characterization of the realizable set (the ladder is necessary, not
sufficient). The skewed analogue of the robustness theorem below is
\cref{thm:cogirth-graded}, a per-rung family of cogirths along this same flag.
...
deferred; we settle it here. The within-class directions are exactly the kernel of the
value quotient: a within-class draw replaces $\enc(v)$ by $\enc(v) \oplus c$ with
$c \in C_{\ell_v} = \ker(\proj)$ restricted to that class, so $\proj$ does not see the
within-class bits at all.

\begin{theorem}[Randomized-encoding invariance and divergence]
\label{thm:randenc}
...
within-class representative $c'(x) = \mathrm{enc\_rand}(v(x)) = \enc(v(x)) \oplus c_x$
with $c_x \in C_{\ell_{v(x)}}$ uniform and independent. Let $\W = \spn\{\cx\}$ and
$\W' = \spn\{c'(x)\}$. Then:
\begin{enumerate}
  \item \emph{(Off-set invisibility.)} $\proj(\W') = \proj(\W)$ exactly, and hence the
  \emph{entire} non-member output law is invariant: $q'(v) = q(v)$ for every value $v$,
  even though the span size $|\W'|$ may differ from $|\W|$. (Proven below for the
  balanced codec; computationally verified for skewed codecs, the flag-level proof being a
  flagged follow-on.)
  ...
  $\bz' \ne \bz$ in general even at a fixed ribbon seed.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{(1, balanced codec.)} Each generator $c'(x) = \cx \oplus c_x$ differs from $\cx$ by
$c_x \in C = \ker(\proj)$, so $\proj(c'(x)) = \proj(\cx)$; since $\proj$ is $\GF$-linear it
carries spans to spans, $\proj(\W') = \spn\{\proj(c'(x))\} = \spn\{\proj(\cx)\} =
\proj(\W)$. For the balanced codec the mass formula simplifies to $q(v) = h_v / |\proj(\W)|$
with $h_v = [\proj(\enc(v)) \in \proj(\W)]$, which depends on $\W$ \emph{only} through
$\proj(\W)$. Hence $q'(v) = q(v)$ for every $v$. The added directions lie in $\ker(\proj)$
and cancel in the normalized mass, so each class mass is pinned even as $|\W'|$ grows.
\emph{(2.)} The build solves $\ay[x]^{\mathsf T}\bz = c'(x)$ rather than $\cx$; a different
right-hand side yields a different back-substituted $\bz$, re-randomized again per build.
No contradiction with (1): (1) concerns $\proj(\W)$, an invariant of the stored values
modulo the within-class bits, while (2) concerns the solved snapshot $\bz$, which depends
on those very bits.
\end{proof}
...
structure. Two consequences follow. (C1) The entire non-member value-frequency channel is
unchanged by within-class randomization, so an attacker who samples non-member outputs
sees identically distributed streams from a canonical and a randomized build (proven
balanced, verified skewed; exact, not asymptotic in either case). (C2) Two builds of the
...
$a^{\mathsf T}\bz$. The \texttt{[randomized]} check gates all three claims at fixed seeds:
exact mass invariance over $1300$ within-class re-encodings across four codecs (asserted by
integer cross-multiplication, while $|\W|$ is observed to grow from $8$ to $16$ on the skew
witness), empirical non-member agreement to total variation $0.0039$ over $30{,}000$
queries, and white-box divergence in about $90\%$ of serialized solution bytes while the
decoded non-member law is unchanged.
...
skewed (variable-length) case is resolved as the \emph{graded} regime (\cref{thm:graded}),
with the skewed cogirth analogue now the per-rung family of \cref{thm:cogirth-graded}; two
finer skewed questions remain open: a complete characterization of the realizable mass set
(the non-increasing ladder is necessary, not sufficient), and the skewed analogue of the
randomized-encoding invariance (\cref{thm:randenc}, proven for balanced and only
computationally verified for skewed). The \emph{magnitude} of the deviation
...
The resolved items leave sharper successors. The skewed codec is now \emph{graded}
(\cref{thm:graded}) and its cogirth budget is the per-rung family of
\cref{thm:cogirth-graded}; what remains is a complete characterization of its realizable mass
set (the ladder is necessary, not sufficient) and the skewed analogue of the
randomized-encoding invariance (\cref{thm:randenc}), which we prove for the balanced codec and
verify computationally in the skewed case. For the
...
\Cref{thm:graded} characterizes the skewed mass vector \eqref{eq:graded} and asserts uniform
is unreachable. The uniform-unreachability dimension argument is given in the proof sketch; the
remaining content is the claim that the realizable set is governed by the ladder and that the
designed Kraft target is achievable while uniform is not, which we verify by exhaustive
enumeration. For a fixed $M$ we enumerate \emph{every} $\GF$ subspace $\W$ of $\GF^M$, compute
...
(iv) the designed Kraft target $q(v) = 2^{-\ell_v}$ is realized (by $\W = \GF^M$); and (v) no
subspace realizes the uniform target, with the nearest realizable vector at total-variation
distance exactly $1/4$. This is the exhaustive backing for \cref{thm:graded}.
