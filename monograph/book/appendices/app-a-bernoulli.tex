...
reported as a non-member. (This $\alpha$ is the Bernoulli false-positive
rate, a different quantity from the acceptance probability $\alpha(y) =
\lvert A(y)\rvert/2^n$ of \Cref{def:acceptance}; for a HashSet the two are
related, the false-positive rate being close to the noise-decode probability
$\varepsilon = \sum_y \alpha(y)$.) Together they are a $2 \times 2$ confusion
matrix,
