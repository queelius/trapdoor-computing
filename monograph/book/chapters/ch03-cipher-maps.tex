...
the list. Hand $\dec$ a uniformly random $12$-bit string instead and it
returns a $0$ or $1$ rather than $\bot$ only with probability $\varepsilon$:
if the construction reserves, say, four of the $4096$ codewords as valid
answers, then $\varepsilon \approx 4/4096 \approx 10^{-3}$, and an
out-of-domain probe is rejected about $999$ times in $1000$. Those ten real
tokens, and the $0.24\%$ support they form, are what the next chapter's
$\delta$ measures uniformity against; the $4096$ ambient strings are not.
\end{example}
