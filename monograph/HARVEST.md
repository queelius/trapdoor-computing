...
**HARVEST STATUS (2026-06-18): the book body + appendices are complete and reviewed; the cross-cut canonical forms are CONSOLIDATED into spine §9 (four cannots, cipher closure, two-scale irreducibility, two-ways-to-hide-frequency, promoted-tables pointer). All back-ports (BP-1..BP-6) DONE. Extraction candidates EC-1 (paradigm) and EC-2 (frontiers) are lift-ready; the remaining work is the actual paper extraction (separate projects) and the per-paper citation passes the 2026-06-09 survey specifies.**

---

## Extraction candidates

**Gap analysis (2026-06-18):** the cross-cutting view identified five papers that would complete the monograph, ranked by how much each *completes* a part (not just extends it): (1) the paradigm paper [EC-1], (2) a formal security-model paper [EC-3, the deepest gap, the framework asserts measurable security but had no security DEFINITION], (3) a systems/application paper (no end-to-end demonstrated system), (4) an adaptive/drift paper (the static-distribution assumption is a missing axis), (5) the matured program-construction paper (ch13 folds a scaffold). #1 and #2 are now DRAFTED (`papers/trapdoor-paradigm/`, `papers/cipher-security-model/`); 3-5 remain open.

| ID | Candidate | From | Kind | Status |
|---|---|---|---|---|
| EC-1 | "Trapdoor Computing: Computing on Opaque Encodings" (the paradigm paper) | Part I | position / overview, ~8-12 pp | **EXTRACTED 2026-06-18 to `papers/trapdoor-paradigm/` (4pp draft).** Part I is DRAFTED and lift-ready (ch1-2 committed, reviewed 2026-06-18). The lift spine is ch1 sec 1.1-1.4 (paradigm, the four cannots, measured-not-negligible, the four claims) + ch2 (the privacy-source thesis and the `tab:not` comparison). The program has no standalone paradigm paper today; this is the gap. Lift with light editing (drop the book roadmap 1.4, add a contributions paragraph). Venues: a security-magazine / CACM-style position piece, or an arXiv overview. |
...
| EC-3 | A formal security-model paper: "A Quantitative Security Model for Trapdoor Computing" | gap analysis 2026-06-18 (the deepest gap) | foundational, QIF security definition | **DRAFTED 2026-06-18 -> `papers/cipher-security-model/` (6pp).** The confidentiality tuple (M marginal / C compositional / A active), the three-scales-one-adversary unification, composition of the tuple, and the defense of QIF vs sim/game security. Answers the cipher-maps review item M1 ("no formal security definition"). New intellectual work, not a lift. |
...
| BP-2 | The four "cannots" (decode / distinguish real-filler / determine domain / tell correct-from-noise) should have one canonical statement the papers cite back to; today it is scattered across cipher-maps sec 1 and the spine. | Part I design (Ch 1.2, Ch 2). | spine (promote a canonical list); cipher-maps + papers cite it | spine §9.1 | **DONE 2026-06-18**: promoted to spine §9.1 (the four cannots) |
| BP-3 | The "what trapdoor computing is not" comparison table (Ch 2.5) is the canonical positioning; consider promoting it into the spine as a shared figure the papers reuse. | Part I design (Ch 2). | spine (shared figure) | spine §9.5 | **DONE 2026-06-18**: promoted to spine §9.5 (tab:not pointer) |
